...
export const OFFICIAL_MPP_TENANTS: MPPTenant[] = [
  ...
  { id: '8da00156-a6fb-4cd7-a7fb-57891fc57294', name: 'Badan Pertanahan Nasional', code: 'BPN', logo: '/logos/bpn.png', floor: 'Lantai 1', is_active: true, description: 'Kantor Pertanahan / ATR-BPN Luwu', created_at: '2026-09-12 10:06:15.467494+00' },
  ...
  { id: '7b256f2c-f84d-482d-a1aa-99825eb7c980', name: 'Bapenda', code: 'BAPENDA', logo: '/logos/bapenda.png', floor: 'Lantai 1', is_active: true, description: 'Badan Pendapatan Daerah Luwu', created_at: '2026-09-12 11:51:56.042785+00' },
  ...
  { id: 'fe2ef49d-f8a7-4c93-8cc9-7af4025e37ce', name: 'Dinas Sosial', code: 'DINSOS', logo: '/logos/dinsos.png', floor: 'Lantai 1', is_active: true, description: 'Dinas Sosial Luwu', created_at: '2026-09-12 11:51:56.982976+00' },
  { id: '074f988a-5c06-4771-a528-b23ff838c722', name: 'Disnakertrans', code: 'DISNAKER', logo: '/logos/disnaker.png', floor: 'Lantai 1', is_active: true, description: 'Dinas Tenaga Kerja dan Transmigrasi Luwu', created_at: '2026-09-12 11:51:57.078252+00' },
  { id: 'ab058d21-f111-4014-a736-785b1e6f0b3f', name: 'Dinas Perikanan', code: 'PERIKANAN', logo: '/logos/perikanan.png', floor: 'Lantai 1', is_active: true, description: 'Dinas Perikanan Luwu', created_at: '2026-09-12 11:51:57.17735+00' },
  ...
  { id: 'b5f466c5-7643-4547-9034-e3420d7031a7', name: 'DEKRANASDA Luwu', code: 'DEKRANASDA', logo: '/logos/dekranasda.png', floor: 'Lantai 1', is_active: true, description: 'Dewan Kerajinan Nasional Daerah Luwu', created_at: '2026-09-12 11:51:57.510238+00' },
...
export const mppService = {
  ...
  async issueQueueTicket(params: {
    ...
    const today = params.queueDate || new Date().toISOString().split('T')[0];
    ...
    // Cari nomor urut terakhir untuk tenant ini pada tanggal ini
    ...
    if (params.citizenName) {
      await supabase.from('mpp_citizens').upsert({
        nik: params.citizenNik,
        full_name: params.citizenName.trim(),
        phone_number: params.citizenPhone || null,
        gender: params.citizenGender || 'Laki-laki',
        jenis_kelamin: params.citizenGender || 'Laki-laki',
        occupation: params.citizenOccupation || 'Masyarakat Pemohon',
        pekerjaan: params.citizenOccupation || 'Masyarakat Pemohon',
        updated_at: new Date().toISOString()
      }, { onConflict: 'nik' });
    }
  ...
  // 6. Pull Antrean Berikutnya dengan Row-Level Locking (Model Teller Bank)
  async pullNextQueue(payload: {
    tenant_id: string;
    served_by?: string;
    served_by_name?: string;
    counter_name?: string;
  }): Promise<{ success: boolean; empty?: boolean; data?: MPPQueue; ticket_code?: string; queue_number?: number; call_count?: number; counter_name?: string; message: string }> {
    try {
      const res = await fetch('/api/mpp/queues/next-call', {
        method: 'POST',
      ...
      return {
        success: false,
        message: errJson.message || 'Gagal memanggil antrean berikutnya.'
      };
    } catch (e: any) {
      return {
        success: false,
        message: e.message || 'Terjadi kesalahan jaringan saat memanggil antrean.'
      };
    }
  },

  // 6B. Call Antrean Spesifik dengan Proteksi Maksimal 3 Kali Panggilan (No-Show Policy)
  async callQueueTicket(
    queueId: string,
    officerInfo?: { served_by?: string; served_by_name?: string; counter_name?: string }
  ): Promise<{ success: boolean; call_count: number; isNoShow: boolean; counter_name?: string; message: string; data?: any }> {
    try {
      const res = await fetch(`/api/mpp/queues/${queueId}/call`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (res.ok) {
        ...
        return {
          success: true,
          call_count: json.call_count || 1,
          isNoShow: false,
          counter_name: json.counter_name,
          message: json.message || 'Berhasil memanggil antrean',
          data: json.data
        };
      }
      ...
      return {
        success: false,
        call_count: errJson.call_count || 0,
        isNoShow: !!errJson.isNoShow,
        message: errJson.message || 'Gagal memanggil tiket antrean'
      };
    } catch {
      // Direct Supabase fallback
      const { data: current } = await supabase
        .from('mpp_queues')
        .select('id, call_count, status, ticket_code, counter_name')
        .eq('id', queueId)
        .single();

      if (!current) {
        return { success: false, call_count: 0, isNoShow: false, message: 'Data antrean tidak ditemukan.' };
      }

      const currentCount = current.call_count || 0;
      if (currentCount >= 3 || current.status === 'tidak_hadir') {
        await supabase
          .from('mpp_queues')
        ...
        return {
          ...
          isNoShow: true,
          message: `Batas maksimal pemanggilan (3 kali) untuk tiket ${current.ticket_code} telah tercapai. Pemohon dinyatakan Tidak Hadir (No-Show).`
        };
      }
      ...
      if (officerInfo?.served_by) updateData.served_by = officerInfo.served_by;
      if (officerInfo?.served_by_name) updateData.served_by_name = officerInfo.served_by_name;
      if (officerInfo?.counter_name) updateData.counter_name = officerInfo.counter_name;
      ...
      return {
        ...
        call_count: nextCount,
        isNoShow: false,
        counter_name: officerInfo?.counter_name || current.counter_name || 'Loket 1',
        message: `Panggilan ke-${nextCount} dari 3 kali untuk tiket ${current.ticket_code}.${nextCount === 3 ? ' Ini adalah panggilan terakhir sebelum dinyatakan Tidak Hadir (No-Show).' : ''}`
      };
    }
  },

  // 7. Update Status Antrean (Dipanggil, Dilayani, Selesai Langsung, Masuk Tracking, Tidak Hadir)
  async updateQueueStatus(
    ...
    if (status === 'dipanggil') {
      const callRes = await this.callQueueTicket(queueId, officerInfo);
    ...
    try {
      const res = await fetch(`/api/mpp/queues/${queueId}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ status, ...(officerInfo || {}) })
      });
      if (res.ok) return true;
    } catch {
      // Ignore network error and proceed with Supabase fallback
    }
    ...
    if (status === 'dilayani') updateObj.served_at = nowIso;
    if (status === 'selesai_langsung' || status === 'masuk_tracking' || status === 'tidak_hadir') updateObj.completed_at = nowIso;
    if (officerInfo?.served_by) updateObj.served_by = officerInfo.served_by;
    if (officerInfo?.served_by_name) updateObj.served_by_name = officerInfo.served_by_name;
    if (officerInfo?.counter_name) updateObj.counter_name = officerInfo.counter_name;
  ...
  // 13. Front Office (FO) - Ambil Daftar Panggilan Bantuan (mpp_fo_requests)
  async getFoRequests(filterStatus?: string): Promise<any[]> {
    try {
      let query = supabase
      ...
      const { data, error } = await query;
      if (error) {
        console.warn('[MPP FO Service] Error fetching FO requests from Supabase:', error);
        // Fallback to server API if direct Supabase query fails
        const res = await fetch(`/api/mpp/fo-requests${filterStatus ? `?status=${filterStatus}` : ''}`);
        if (res.ok) {
          const json = await res.json();
          return json.data || [];
        }
      ...
      console.error('[MPP FO Service] fetch error:', err);
      return [];
    }
  },

  // 14. Front Office (FO) - Buat Panggilan Bantuan Baru
  async createFoRequest(params: {
    sourceType: string;
    sourceName: string;
    notes?: string;
  }): Promise<{ success: boolean; data?: any; error?: string }> {
    try {
      // Normalize source_type strictly to database CHECK constraint: ('layanan_mandiri', 'gerai_tenant')
      const rawType = (params.sourceType || '').toLowerCase();
      ...
      const payload = {
        ...
        source_name: params.sourceName || (normalizedSourceType === 'gerai_tenant' ? 'Gerai Pelayanan' : 'Kiosk Layanan Mandiri'),
        status: 'pending',
      ...
      if (error) {
        ...
        const res = await fetch('/api/mpp/fo-requests', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload)
        });
      ...
      return { success: false, error: err.message || 'Gagal mengirim permintaan bantuan' };
    }
  },

  // 15. Front Office (FO) - Tanggapi (OTW) / Selesaikan Panggilan
  async updateFoRequestStatus(params: {
    id: string;
    ...
    try {
      ...
      if (params.status === 'resolved') {
        ...
        if (params.operatorId) {
          updateData.resolved_by = params.operatorId;
        }
      }
      ...
      if (error) {
        const res = await fetch(`/api/mpp/fo-requests/${params.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
      ...
      return { success: false, error: err.message || 'Gagal memperbarui status permintaan' };
    }
  },

  // 16. Front Office (FO) - Ambil Petugas FO dari operators
  async getFoOperators(): Promise<any[]> {
    try {
      const { data, error } = await supabase
        .from('operators')
      ...
      // Query all operators if role front_office filter returns empty
  ...
  // 17. e-Teguran (Real-time Reprimand System) - Ambil Daftar Teguran (bisa filter per tenant, user, loket, status)
  async getReprimands(options?: {
    tenantId?: string;
    status?: string;
    target_user_id?: string;
    target_counter_name?: string;
  } | string, maybeStatus?: string): Promise<MppReprimand[]> {
    try {
      let tenantId: string | undefined;
      let status: string | undefined;
      let target_user_id: string | undefined;
      let target_counter_name: string | undefined;
      ...
      if (typeof options === 'string') {
        ...
        target_counter_name = options.target_counter_name;
      }
      ...
      if (target_counter_name) {
        query = query.eq('target_counter_name', target_counter_name);
      }

      const { data, error } = await query;

      if (error) {
        // Fallback to Express backend if direct Supabase query errors
        const params = new URLSearchParams();
        if (tenantId) params.append('tenant_id', tenantId);
        if (status) params.append('status', status);
        if (target_user_id) params.append('target_user_id', target_user_id);
        if (target_counter_name) params.append('target_counter_name', target_counter_name);
        const res = await fetch(`/api/mpp/reprimands?${params.toString()}`);
        if (res.ok) {
          const json = await res.json();
          return json.data || [];
        }
  ...
  // 18. e-Teguran - Kirim Teguran Baru (Kepala DPMPTSP / Admin MPP) dengan opsi target Petugas/Loket spesifik
  async createReprimand(payload: {
    tenant_id: string;
  ...
  }): Promise<{ success: boolean; data?: MppReprimand; error?: string }> {
    try {
      const insertData = {
        tenant_id: payload.tenant_id,
        target_user_id: payload.target_user_id || null,
        target_counter_name: payload.target_counter_name || null,
        target_officer_name: payload.target_officer_name || null,
        quick_reasons: payload.quick_reasons || [],
      ...
      if (error) {
        ...
        const res = await fetch('/api/mpp/reprimands', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      return { success: false, error: err.message || 'Gagal mengirim teguran' };
    }
  },

  // 19. Ambil Daftar Petugas Loket per Gerai (Multi-Counter User Management)
  async getTenantOfficers(tenantId: string): Promise<MPPTenantUser[]> {
    try {
      const { data, error } = await supabase
  ...
  // 20. e-Teguran - Konfirmasi/Wajib Acknowledgment oleh Petugas Gerai
  async acknowledgeReprimand(id: string, readByName: string): Promise<{ success: boolean; data?: MppReprimand; error?: string }> {
    try {
      const updateData = {
        status: 'read',
        read_by_name: readByName.trim(),
      ...
      if (error) {
        // Fallback to Express API
        const res = await fetch(`/api/mpp/reprimands/${id}/acknowledge`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ read_by_name: readByName })
        });
        if (res.ok) {
          const json = await res.json();
          return { success: true, data: json.data };
      ...
      return { success: false, error: err.message || 'Gagal memproses konfirmasi' };
    }
  },

  // 21. e-Teguran - Tandai Selesai / Ditindaklanjuti
  async resolveReprimand(id: string): Promise<{ success: boolean; error?: string }> {
    try {
      const { error } = await supabase
        .from('mpp_reprimands')
      ...
      if (error) {
        const res = await fetch(`/api/mpp/reprimands/${id}/resolve`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' }
        });
        if (res.ok) return { success: true };
      ...
      return { success: false, error: err.message || 'Gagal menyelesaikan teguran' };
    }
  }
};
