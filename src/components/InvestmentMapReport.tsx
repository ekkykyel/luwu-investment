...
import { District, Investment, Village, SektorInvestasi, GeoJSONLayer } from "../types.js";
...
import { MapSnapshot } from "../types.js";
...
export default function InvestmentMapReport({
    ...
    return (
      <div className="fixed inset-0 z-[99999] bg-slate-900 flex items-center justify-center text-white font-sans p-6">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-400">Silakan tunggu, sistem sedang mempersiapkan data.</p>
        </div>
      </div>
    );
  }
  ...
  const renderCrest = (isMainKop = false) => (
    <div className={`relative ${isMainKop ? 'w-10 h-10' : 'w-7 h-7'} flex items-center justify-center shrink-0`}>
      <img src="https://i.ibb.co.com/KxKKb5d8/transparant.png" style={{ width: isMainKop ? "40px" : "28px", height: isMainKop ? "40px" : "28px" }} className="object-contain" alt="Logo Luwu" />
    </div>
  );
                    ...
                    <tbody className="divide-y divide-black/10 font-sans text-[#000000]">
                      {(() => {
                        ...
                        if (!finalRoi) {
                          return (
                            <tr>
                              <td colSpan={4} className="p-4 text-center font-bold text-slate-500 italic">
