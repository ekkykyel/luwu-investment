...
export default function MppQueueRegistrationModal({ isOpen, onClose, isDarkMode }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data: lastQueue, error: queueErr } = await supabase
        .from('mpp_queues')
        .select('queue_number')
      ...
      if (lastQueue && lastQueue.length > 0) {
        nextNum = lastQueue[0].queue_number + 1;
      }

      // Generate Ticket Code (e.g. DPMPTSP-20261012-001 or P-DPMPTSP-20261012-001 for priority lane)
      const dateStr = today.replace(/-/g, '');
      const paddedNum = nextNum.toString().padStart(3, '0');
      const ticketCode = isPriorityLane 
        ? `P-${selectedTenant.code}-${dateStr}-${paddedNum}`
        : `${selectedTenant.code}-${dateStr}-${paddedNum}`;

      // 3. Insert Queue
      const { data: newQueue, error: insertError } = await supabase.from('mpp_queues').insert({
      ...
      <div className={`relative w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col overflow-hidden ${isDarkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-900'}`}>
        ...
        <div className={`px-6 py-4 border-b flex justify-between items-center ${isDarkMode ? 'border-slate-800' : 'border-slate-200'}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold">Ambil Antrian MPP</h2>
              <p className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Portal Layanan Publik Terpadu</p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto max-h-[70vh] custom-scrollbar">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {tenants.map(tenant => (
                    ...
                    >
                      ...
                      <div className="text-xs text-slate-500 mt-1">Kode: {tenant.code}</div>
                    </button>
                  ))}
                </div>
              )}
          ...
          {step === 2 && (
            <div className="space-y-4">
              <button onClick={() => setStep(1)} className="text-xs text-indigo-500 hover:underline mb-2 block">&larr; Kembali ke pilihan gerai</button>
              <h3 className="font-semibold text-lg flex items-center gap-2">
                <FileText className="w-5 h-5 text-indigo-500" />
                Pilih Layanan di {selectedTenant?.name}
              </h3>
              {services.length === 0 ? (
                <div className="text-center py-8 text-slate-500">
                  Belum ada layanan yang didaftarkan untuk gerai ini.
                </div>
              ) : (
                  ...
                  {services.map(service => (
                    ...
                    >
                      <div>
                        ...
                        {service.is_long_process && <span className="inline-block mt-1 px-2 py-0.5 rounded text-[10px] font-bold bg-amber-500/10 text-amber-600 border border-amber-500/20">Butuh Tracking / E-LACAK</span>}
                      </div>
          ...
          {step === 3 && (
            <div className="space-y-4">
               <button onClick={() => setStep(2)} className="text-xs text-indigo-500 hover:underline mb-2 block cursor-pointer">&larr; Kembali ke pilihan layanan</button>
               <h3 className="font-semibold text-lg flex items-center gap-2">
                <User className="w-5 h-5 text-indigo-500" />
                Data Pemohon Antrean
              </h3>
                ...
                <div>
                    ...
                    <div className="flex items-center gap-1.5">
                      {isSearchingNik ? (
                        <span className="text-[10px] text-indigo-500 flex items-center gap-1">
                          <Loader2 className="w-2.5 h-2.5 animate-spin" /> Cek database...
                        </span>
                      ) : isNikValid(nik) ? (
                  ...
                  <div className="flex gap-2">
                    <input 
                      ...
                      onChange={(e) => {
                        ...
                        if (raw.length === 0) {
                          setFormErrors(prev => ({ ...prev, nik: 'NIK wajib diisi sesuai KTP' }));
                        } else if (raw.length < 16) {
                          setFormErrors(prev => ({ ...prev, nik: `NIK harus 16 digit angka (saat ini: ${raw.length}/16)` }));
                        } else {
                  ...
                  {touched.nik && formErrors.nik ? (
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      {nik.startsWith('7317') ? '📍 Wilayah Kabupaten Luwu (7317)' : 'Masukkan 16 digit NIK. Sistem Tell Us Once akan mengisi data otomatis jika sudah pernah terdaftar.'}
                    </p>
                  )}
                </div>
