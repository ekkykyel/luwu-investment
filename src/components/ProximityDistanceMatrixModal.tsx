...
import { formatRupiahSingkat, formatRupiah as globalFormatRupiah } from "../lib/formatters";
import { useTranslation } from "react-i18next";
import { calculateShortestPathGeoJSON } from "../utils/routeService";
import { getHydrologyByDistrictName } from "../utils/luwuHydrology";
...
export default function ProximityDistanceMatrixModal({
  ...
  const isMountainLocation =
    ...
    districtName.includes("basse sangtempe") ||
  ...
  const distanceMatrixResults = useMemo(() => {
    ...
    return LUWU_INFRASTRUCTURE_NODES.map((node) => {
      ...
      if (apiSpatialData?.distances) {
        ...
        } else if (activeNode.id === "kib_bua" && d.nearestKib?.distanceKm) {
          roadDistKm = d.nearestKib.distanceKm;
          routingMethod = d.nearestKib.isNetworkRouting ? "NETWORK" : "ESTIMATED";
        } else if (activeNode.id === "agro_bastem" && d.nearestAgro?.distanceKm) {
          roadDistKm = d.nearestAgro.distanceKm;
          routingMethod = d.nearestAgro.isNetworkRouting ? "NETWORK" : "ESTIMATED";
        }
      }
      ...
      let statusColor = "text-emerald-700 dark:text-emerald-400 border-emerald-500/30 bg-emerald-500/10";
      if (roadDistKm > 40) {
        status = "TERJANGKAU";
          ...
          <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-0.5">
                ...
                <span className={`text-[10px] sm:text-xs whitespace-nowrap ${isDark ? "text-slate-600 dark:text-slate-400" : "text-slate-600 dark:text-slate-400"}`}>{t('matrix_network_luwu', 'Jaringan Infrastruktur Luwu')}</span>
              </div>
        ...
        <div className={`p-3.5 sm:p-5 md:p-6 overflow-y-auto space-y-4 sm:space-y-6 flex-1 scrollbar-thin ${isDark ? "scrollbar-thumb-neutral-800 bg-black" : "scrollbar-thumb-slate-200 bg-slate-50/50"}`}>
          ...
          {simulationContext && (
            <div className={`p-4 rounded-2xl border shadow-lg space-y-3 ${
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 font-mono">
                ...
                <div className="p-3 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm text-center flex flex-col items-center justify-center">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1 block font-sans">IRR Proyek</span>
                  <span className="text-base sm:text-lg font-extrabold text-slate-900 dark:text-white leading-tight">{(Number(simulationContext.irr) || 0).toFixed(2)}%</span>
                </div>
                <div className="p-3 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm text-center flex flex-col items-center justify-center">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1 block font-sans">{t('payback_period', 'BEP Payback')}</span>
                  <span className="text-base sm:text-lg font-extrabold text-slate-900 dark:text-white leading-tight">{(Number(simulationContext.bep) || 0).toFixed(2)} Thn</span>
                </div>
              </div>
          ...
          <div className={`p-3.5 sm:p-4 rounded-2xl border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-md ${
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <span className={`text-[10px] font-bold uppercase tracking-wider block ${isDark ? "text-slate-600 dark:text-slate-400" : "text-slate-600 dark:text-slate-400"}`}>
                ...
                <span className={`text-[11px] block sm:inline ${isDark ? "text-slate-600 dark:text-slate-400" : "text-slate-600"}`}>
            ...
            <div className={`w-full sm:w-auto px-4 py-2 border border-emerald-500/30 rounded-xl text-left sm:text-right shrink-0 flex sm:block items-center justify-between ${
              ...
              <span className={`text-[10px] font-bold uppercase ${isDark ? "text-slate-600 dark:text-slate-400" : "text-slate-600"}`}>
          ...
          <div className={`p-2.5 sm:p-3 rounded-2xl border flex flex-col sm:flex-row gap-2 sm:items-center ${
            ...
            <span className={`text-[10px] font-bold uppercase tracking-wider shrink-0 px-1 ${isDark ? "text-slate-600 dark:text-slate-400" : "text-slate-600"}`}>
                  ...
                  className={`px-3 py-2 min-h-[36px] rounded-xl text-xs font-semibold whitespace-nowrap transition-all cursor-pointer snap-start active:scale-95 shrink-0 ${
                    selectedCategory === cat.key
                      ...
                      : isDark 
                        ...
                        : "bg-slate-100 text-slate-800 dark:text-slate-200 hover:text-slate-900 border border-slate-200"
                  }`}
                >
                  {cat.label}
                </button>
              ...
              return (
                ...
                >
                    ...
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 mt-0.5">
                          <span className={`text-[10px] font-medium ${isDark ? "text-slate-600 dark:text-slate-400" : "text-slate-600 dark:text-slate-400"}`}>
                  ...
                  <div className={`px-2.5 py-1.5 rounded-xl border text-[10px] flex items-center justify-between ${
                    isDark ? "bg-black/80 border-neutral-800 text-slate-300" : "bg-slate-50 border-slate-200 text-slate-800 dark:text-slate-200"
                  }`}>
                    <span className={`${isDark ? "text-slate-600 dark:text-slate-400" : "text-slate-600 dark:text-slate-400"} font-medium`}>{t('metric_operational_capacity', 'Kapasitas Operasional:')}</span>
                  ...
                  <div className={`grid grid-cols-3 gap-1.5 sm:gap-2 pt-2.5 border-t text-center ${isDark ? "border-neutral-800/80" : "border-slate-200"}`}>
                    <div className={`p-2 rounded-xl border ${isDark ? "bg-black border-neutral-800/60" : "bg-slate-50 border-slate-200"}`}>
                      <span className={`text-[9px] font-bold uppercase block whitespace-nowrap ${isDark ? "text-slate-600 dark:text-slate-400" : "text-slate-600 dark:text-slate-400"}`}>
                      ...
                      <span className={`text-[9px] block ${isDark ? "text-slate-600 dark:text-slate-400" : "text-slate-600 dark:text-slate-400"}`}>
          ...
          <div className={`p-4 rounded-2xl border text-xs space-y-3 shadow-lg ${
            isDark 
              ...
              : "bg-gradient-to-br from-slate-100 via-white to-slate-100 border-amber-500/30 text-slate-800 dark:text-slate-200"
          }`}>
            <div className={`flex items-center gap-2.5 border-b pb-2.5 ${isDark ? "border-neutral-800" : "border-slate-200"}`}>
              ...
              <div>
                ...
                <span className={`text-[10px] block ${isDark ? "text-slate-600 dark:text-slate-400" : "text-slate-600 dark:text-slate-400"}`}>
