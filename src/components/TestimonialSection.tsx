    ...
    <div className={`w-full py-16 sm:py-24 ${isDark ? 'bg-slate-900/50' : 'bg-slate-50'}`}>
      <div className="container max-w-7xl mx-auto px-4 lg:px-6">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>{t('testimonials.subtitle', 'Tanggapan dan cerita sukses dari pelaku usaha di Kabupaten Luwu')}</p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 px-4 text-slate-500 border border-slate-800 border-dashed rounded-2xl max-w-lg mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {testimonials.map((item, i) => (
              ...
              >
                ...
                <p className={`text-sm italic mb-6 leading-relaxed ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>"{item.message}"</p>
                <div>
                  <div className="flex items-center gap-2">
                    <h4 className={`text-sm font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{item.investor_name || item.company_name}</h4>
                    {item.is_verified && (
                      <div className="flex items-center text-blue-400 text-xs gap-1" title={t('testimonial.verified', 'Terverifikasi')}>
                        <BadgeCheck size={14} />
