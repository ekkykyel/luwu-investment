...
export default function HeroSettings() {
  ...
  const fetchCurrentHeroes = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .eq("setting_key", "hero_slider_images")
        .single();
      ...
      if (data && data.setting_value) {
         ...
         const { data: oldData } = await supabase.from("site_settings").select("setting_value").eq("setting_key", "hero_image_url").single();
         if (oldData && oldData.setting_value) {
           setHeroImages([oldData.setting_value]);
         }
      }
    } catch (error) {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto p-4 sm:p-6 bg-[#020617] min-h-screen text-slate-50 font-sans">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-emerald-400 mb-2">Upload Photo Slider</h1>
        <p className="text-sm text-slate-400">Kelola slider visual utama landing page (Maksimal 5 photo).</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#0f172a] rounded-xl border border-slate-800 p-5 shadow-xl flex flex-col gap-4">
           ...
           {isLoading && heroImages.length === 0 ? (
              ...
              <div className="h-40 border border-slate-800 rounded bg-slate-900 border-dashed flex items-center justify-center text-slate-500 text-xs p-4 text-center">
          ...
          >
            ...
            {heroImages.length >= 5 ? (
              ...
              <>
                ...
                <span className="text-xs text-slate-500 mt-1">.webp, .jpg, .png (Max 5MB)</span>
