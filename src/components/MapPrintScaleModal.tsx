...
export default function MapPrintScaleModal({ 
  ...
  const captureMap = async () => {
    ...
    try {
      ...
      if (map) {
        ...
        // Trigger repaint and capture on next animation frame for ultra-fast snapshotting
        await new Promise<void>(resolve => {
            ...
            try {
              ...
              if (mCanvas) {
                ...
                if (ctx) {
                  ...
                  snapDataUrl = temp.toDataURL("image/jpeg", 0.92);
                } else {
          ...
          map.triggerRepaint();
          requestAnimationFrame(() => setTimeout(doCapture, 150));
        });
      }
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      // Fast, high-precision layout snapshot at scale: 2 for crisp A4 PDF output
      const canvas = await pdfRenderQueue.enqueue(() => safeHtml2Canvas(printLayoutRef.current!, {
        scale: 2, 
        useCORS: true,
        allowTaint: true,
        logging: false,
      ...
      >
        ...
        <div className="w-full lg:w-80 bg-white border-b lg:border-b-0 lg:border-r border-slate-200 flex flex-col p-6 shrink-0 z-10 shadow-lg">
          <div className="flex items-center justify-between mb-5">
            ...
            <button onClick={onClose} className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-600 dark:text-slate-400 cursor-pointer">
              <X className="h-5 w-5" />
            </button>
          </div>
          ...
          <div className="flex flex-col gap-5">
            ...
            <div>
              <label className="text-[10px] font-bold font-mono tracking-widest uppercase text-slate-600 dark:text-slate-400 mb-2 block">
                Orientasi Kertas A4
              </label>
            ...
            <div>
              ...
              <p className="text-[9px] text-slate-600 dark:text-slate-400 mt-1.5 leading-relaxed italic">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 border border-slate-200 bg-slate-50 hover:bg-slate-100 active:scale-95 text-slate-800 dark:text-slate-200 text-xs font-bold rounded-xl transition-all cursor-pointer disabled:opacity-50"
            >
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-4 border-b-[3px] border-slate-900 pb-2 mb-3 shrink-0">
                <img src={LUWU_LOGO_BASE64} alt="Logo Luwu" className="h-10 w-10 object-contain" />
              ...
              <div className="flex-1 border border-slate-800 bg-slate-50 relative overflow-hidden flex items-center justify-center">
                {isCapturing ? (
                  <div className="flex flex-col items-center justify-center text-slate-600 dark:text-slate-400 gap-2">
                  ...
                  <span className="text-[9px] text-slate-600 dark:text-slate-400 font-mono">Belum ada tangkapan peta</span>
                )}
                ...
                <div className="flex-1 border-r border-slate-300 pr-4">
                  ...
                  <div className="grid grid-cols-2 gap-x-3 gap-y-1">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[7px] font-mono text-slate-800 dark:text-slate-200">Batas Administrasi</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <div className="h-2.5 w-2.5 border border-blue-600 bg-blue-500/20 rounded-sm"></div>
                      <span className="text-[7px] font-mono text-slate-800 dark:text-slate-200">Area Pesisir & Laut</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <div className="h-0.5 w-3 bg-orange-500"></div>
                      <span className="text-[7px] font-mono text-slate-800 dark:text-slate-200">Jaringan Jalan Raya</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <div className="h-2.5 w-2.5 border border-emerald-600 bg-emerald-500/20 rounded-sm"></div>
                      <span className="text-[7px] font-mono text-slate-800 dark:text-slate-200">Tutupan Lahan Hijau</span>
                    </div>
                  </div>
                </div>
                <div className="w-44 flex flex-col justify-end text-[6px] font-mono text-right text-slate-600 dark:text-slate-400 leading-tight">
                  <p>Sumber Data:</p>
