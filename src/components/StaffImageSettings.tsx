...
export default function StaffImageSettings() {
  ...
  const fetchCurrentStaffImages = async () => {
    ...
    try {
      ...
      if (data) {
        const leftData = data.find(d => d.setting_key === "staff_image_left");
        const rightData = data.find(d => d.setting_key === "staff_image_right");
        ...
        >
          ...
          {previewUrl ? (
            ...
            <>
              ...
              <span className="text-[10px] text-slate-500 mt-1">.webp, .jpg, .png (Max 5MB)</span>
            </>
          )}
        </motion.div>
  ...
  return (
    <div className="w-full max-w-5xl mx-auto p-4 sm:p-6 bg-[#020617] min-h-screen text-slate-50 font-sans">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-emerald-400 mb-2">Upload Photo Staff</h1>
        <p className="text-sm text-slate-400">Kelola gambar profil staff (kiri dan kanan) yang melayang di beranda.</p>
      </div>
