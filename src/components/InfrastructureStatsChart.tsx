...
export default function InfrastructureStatsChart({ infrastructure = [], isDarkMode }: InfrastructureStatsChartProps) {
  ...
  return (
    <div className={`p-5 sm:p-6 rounded-2xl backdrop-blur-md border shadow-2xl transition-all duration-300 relative z-10 ${
      isDarkMode ? "bg-slate-900/80 border-slate-700/50 text-white" : "bg-white/80 border-white/40 text-slate-800"
    }`} id="infra-stats-section">
      
      {/* Header and Controls */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 border-b pb-3 border-slate-900/10 dark:border-white/10">
        <div>
          <h4 className={`text-xs font-display font-bold uppercase tracking-wider ${isDarkMode ? "text-slate-100" : "text-slate-900"}`}>
            {t("infraStats.title", "Statistik Infrastruktur Luwu")}
          </h4>
          <span className={`text-[10px] sm:text-[10.5px] block font-sans ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>
      ...
      <div className="h-56 w-full relative flex items-center justify-center">
        {aggregatedData.length === 0 ? (
          <div className="text-center italic py-6 flex flex-col items-center gap-2 font-sans">
            ...
            <span className={`text-[9.5px] ${isDarkMode ? "text-slate-500" : "text-slate-400"}`}>
          ...
          {aggregatedData.map((item, idx) => (
            <div key={`${item.name}-${idx}`} className="flex items-center gap-2 min-w-0">
              ...
              <div className="flex flex-col min-w-0 flex-1">
                <span className={`text-[10px] truncate font-medium leading-tight ${isDarkMode ? "text-slate-300" : "text-slate-700"}`} title={item.name}>
                  {item.name}
                </span>
                <span className={`text-[8.5px] font-mono leading-none ${isDarkMode ? "text-slate-500" : "text-slate-400"}`}>
