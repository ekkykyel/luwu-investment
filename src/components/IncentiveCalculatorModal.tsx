...
export default function IncentiveCalculatorModal({
  ...
  const initialCapex =
    ...
    25000000000; // Default 25 Miliar

  const [capexInput, setCapexInput] = useState<number>(initialCapex);
  const [capexDisplay, setCapexDisplay] = useState<string>(() =>
    initialCapex ? new Intl.NumberFormat("id-ID").format(initialCapex) : ""
  );

  const [laborLocalPercent, setLaborLocalPercent] = useState<number>(75); // % TKD lokal Luwu
  const [selectedSector, setSelectedSector] = useState<string>(
    simulationContext?.sector || (selectedInvestment as any)?.sector || (selectedInvestment as any)?.sektor || "Kakao & Agroindustri"
  );
  ...
  useEffect(() => {
    if (simulationContext) {
      const simCapex = simulationContext.capex || (selectedInvestment as any)?.investmentValue || 25000000000;
      ...
      const invCapex = (selectedInvestment as any)?.investmentValue || (selectedInvestment as any)?.nilaiInvestasi || 25000000000;
  ...
  const incentiveResult = useMemo(() => {
    ...
    const slaDays = capexInBillions >= 10 ? "1 Hari Kerja (Fast-Track DPMPTSP)" : "2 Hari Kerja";
        ...
        <div className={`p-3.5 sm:p-5 md:p-6 overflow-y-auto space-y-4 sm:space-y-6 flex-1 scrollbar-thin ${
            ...
            <div className={`p-4 rounded-2xl border shadow-lg space-y-3 ${
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 font-mono">
                <div className={`p-2.5 rounded-xl border text-center ${isDark ? "bg-black/80 border-neutral-800" : "bg-white border-slate-200 shadow-xs"}`}>
                  ...
                  <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400">{formatRupiah(simulationContext.capex)}</span>
                </div>
                <div className={`p-2.5 rounded-xl border text-center ${isDark ? "bg-black/80 border-neutral-800" : "bg-white border-slate-200 shadow-xs"}`}>
                ...
                <div className={`p-2.5 rounded-xl border text-center ${isDark ? "bg-black/80 border-neutral-800" : "bg-white border-slate-200 shadow-xs"}`}>
                  <span className={`text-[9px] block uppercase font-sans ${isDark ? "text-slate-400" : "text-slate-500"}`}>{t('incentiveCalculator.npvDiscount', 'NPV (Discount 9%)')}</span>
                  <span className="text-xs font-bold text-indigo-600 dark:text-indigo-300">{formatRupiah(simulationContext.npv)}</span>
                </div>
                <div className={`p-2.5 rounded-xl border text-center ${isDark ? "bg-black/80 border-neutral-800" : "bg-white border-slate-200 shadow-xs"}`}>
                  ...
                  <span className="text-xs font-bold text-cyan-600 dark:text-cyan-300">{(Number(simulationContext.bep) || 0).toFixed(2)} {t('incentiveCalculator.years', 'Thn')}</span>
                </div>
              </div>
              ...
              {(() => {
                ...
                return (
                  <div className={`p-3 rounded-xl border text-xs leading-relaxed font-sans ${
                    isDark ? "bg-amber-500/10 border-amber-500/20 text-amber-200/90" : "bg-amber-100/80 border-amber-300 text-amber-950"
                  }`}>
          ...
          <div className={`p-3.5 sm:p-5 rounded-2xl border space-y-4 shadow-md ${
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5 sm:gap-4">
              ...
              <div>
                <label className={`text-[11px] sm:text-[10px] font-bold uppercase block mb-1 ${isDark ? "text-slate-300" : "text-slate-700"}`}>
                ...
                <div className="relative flex items-center">
                  ...
                  <input
                    ...
                    className={`w-full border rounded-xl pl-9 pr-3.5 h-12 sm:h-10 text-sm sm:text-xs font-bold focus:outline-none tracking-wide font-mono ${
                      isDark 
                        ? "bg-black border-neutral-800 text-emerald-400 focus:border-amber-500/80" 
                        : "bg-slate-50 border-slate-300 text-emerald-800 focus:border-amber-600"
                    }`}
                  />
                </div>
                <div className={`flex items-center justify-between gap-1 mt-1 text-[10px] font-medium ${isDark ? "text-slate-400" : "text-slate-500"}`}>
              ...
              <div>
                ...
                <span className={`text-[10px] block mt-0.5 font-medium ${isDark ? "text-slate-400" : "text-slate-500"}`}>
              ...
              <div className="sm:col-span-2 lg:col-span-1">
                ...
                >
                  ...
                  <option value="Budidaya Cengkeh & Kopi">Cengkeh & Kopi Bastem</option>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
            {/* Retribusi Diskon */}
            <div className={`p-4 sm:p-5 rounded-2xl border relative overflow-hidden space-y-1 ${
              ...
              <span className={`text-[11px] block leading-snug pt-1 ${isDark ? "text-slate-400" : "text-slate-600"}`}>
                {t('discount_retribusi_desc', { amount: formatRupiah(incentiveResult.pbgSavings) })}
              </span>
            </div>
          ...
          <div className={`p-4 sm:p-5 rounded-2xl border space-y-3 ${
            ...
            <div className={`flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b pb-3 ${
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <span className={`text-[11px] block ${isDark ? "text-slate-300" : "text-slate-700"}`}>
                    {t('basic_deduction_desc', 'Potongan Dasar Retribusi Daerah & Meringankan Sewa Barang Milik Daerah (BMD)')}
                  </span>
                </div>
              </div>
              <div className="text-left sm:text-right shrink-0">
                <span className={`text-[10px] block ${isDark ? "text-slate-400" : "text-slate-500"}`}>{t('est_nominal_incentive', 'Estimasi Nominal Insentif:')}</span>
            ...
            <div className={`space-y-2 text-xs ${isDark ? "text-slate-300" : "text-slate-700"}`}>
              ...
              <ul className={`space-y-1.5 pl-5 list-disc text-[11px] leading-relaxed ${isDark ? "text-slate-300" : "text-slate-700"}`}>
                <li>
