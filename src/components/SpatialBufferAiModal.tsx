...
import { District, Village } from "../types.js";
  ...
  useEffect(() => {
    if (isOpen) {
      if (selectedInvestment) {
        setMessages([
          { role: "model" as const, text: selectedGreetings[aiLanguage](selectedInvestment.name, selectedInvestment.sector) }
        ]);
        setActiveTab("ai");
      } else {
      ...
      parts.push(
        <span 
          ...
          className={`inline-flex items-center gap-1 sm:gap-1.5 align-middle mx-1 px-2 py-0.5 sm:px-2.5 sm:py-1 border rounded-lg font-mono text-[9px] sm:text-[10px] cursor-pointer hover:scale-105 active:scale-95 transition-all shadow-md group ${
            isDarkMode 
              ? "bg-emerald-950/40 border-emerald-500/50 text-emerald-400 hover:bg-emerald-900/60 shadow-[0_0_10px_rgba(52,211,153,0.3)] hover:shadow-[0_0_15px_rgba(52,211,153,0.5)]" 
        ...
        <div className={`flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between px-4 sm:px-5 py-3 sm:py-4 border-b shrink-0 ${
          ...
          <div className="flex items-center gap-2 sm:gap-3 overflow-hidden">
            <div className={`p-1.5 sm:p-2 border rounded-xl shrink-0 ${
              ...
              <Sparkles className={`h-4.5 w-4.5 sm:h-5 sm:w-5 ${isDarkMode ? "text-blue-400" : "text-blue-600"}`} />
            </div>
            <div className="overflow-hidden">
              ...
              <p className={`text-[8px] sm:text-[10px] font-sans truncate ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>AI Investment Site Recommendation & Spatial Analytics</p>
            </div>
          </div>
        ...
        <div className={`flex border-b shrink-0 ${
          ...
          <button 
            ...
            className={`flex-1 py-3 text-[10px] sm:text-xs font-bold font-display uppercase tracking-wider flex justify-center items-center gap-1.5 border-b-2 transition-colors ${
              activeTab === "buffer" 
                ? (isDarkMode ? "border-emerald-500 text-emerald-400 bg-slate-800/50" : "border-emerald-600 text-emerald-700 bg-white") 
                : (isDarkMode ? "border-transparent text-slate-500 hover:text-slate-300" : "border-transparent text-slate-500 hover:text-slate-700")
            }`}
          >
          ...
          <button 
            ...
            className={`flex-1 py-3 text-[10px] sm:text-xs font-bold font-display uppercase tracking-wider flex justify-center items-center gap-1.5 border-b-2 transition-colors ${
              activeTab === "ai" 
                ? (isDarkMode ? "border-blue-500 text-blue-400 bg-slate-800/50" : "border-blue-600 text-blue-700 bg-white") 
                : (isDarkMode ? "border-transparent text-slate-500 hover:text-slate-300" : "border-transparent text-slate-500 hover:text-slate-700")
            }`}
          >
              ...
              <div className={`border rounded-[20px] p-4 sm:p-5 flex flex-col gap-4 shadow-xl transition-all ${
                ...
                <div className="grid grid-cols-2 gap-3 sm:gap-4">
                  <div className="flex flex-col gap-1.5 relative">
                    <label className={`text-[8px] sm:text-[9px] font-mono uppercase tracking-widest pl-1 ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>Latitude (Y)</label>
                    <input 
                      type="number" 
                      step="any" 
                      value={bufferLat} 
                  ...
                  <div className="flex flex-col gap-1.5 relative">
                    <label className={`text-[8px] sm:text-[9px] font-mono uppercase tracking-widest pl-1 ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>Longitude (X)</label>
                    <input 
                      type="number" 
                      step="any" 
                      value={bufferLng} 
                ...
                <div className="flex flex-col gap-2.5 mt-1">
                  <div className="flex items-center justify-between">
                    <label className={`text-[8px] sm:text-[9px] font-mono uppercase tracking-widest pl-1 ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>{t('spatial.buffer')}</label>
                    <span className={`font-mono text-[10px] sm:text-[11px] font-bold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-lg border ${
                      isDarkMode 
                        ? "text-emerald-400 bg-emerald-950/50 border-emerald-900/80" 
              ...
              <div className="flex flex-col gap-2 flex-1 mt-1">
                ...
                {bufferResults.hasRun ? (
                    ...
                    isDarkMode 
                     ...
                     <div className="flex items-center justify-between border-b pb-3 relative z-10 border-slate-500/10">
                        <div className="flex flex-col gap-0.5">
                          <span className={`text-[8px] font-mono uppercase tracking-wider flex items-center gap-1 ${isDarkMode ? "text-emerald-400" : "text-emerald-700"}`}><Sparkles className="h-2.5 w-2.5" /> AI Readiness Score</span>
                     ...
                     <div className="grid grid-cols-2 gap-3 relative z-10">
                        <div className={`border rounded-xl p-2.5 shadow-sm ${isDarkMode ? "bg-slate-950/80 border-slate-800" : "bg-white border-slate-200"}`}>
                          <span className="text-[8px] text-slate-500 font-mono uppercase tracking-widest block">Populasi Target</span>
                          <span className={`text-sm sm:text-base font-bold font-mono block tracking-tight ${isDarkMode ? "text-indigo-400" : "text-indigo-700"}`}>
                            {bufferResults.impactedVillages.reduce((sum, v) => sum + v.population, 0).toLocaleString()} <span className="text-[8px] font-sans text-slate-500 font-normal">Jiwa</span>
                          </span>
                        </div>
                        <div className={`border rounded-xl p-2.5 shadow-sm ${isDarkMode ? "bg-slate-950/80 border-slate-800" : "bg-white border-slate-200"}`}>
                          <span className="text-[8px] text-slate-500 font-mono uppercase tracking-widest block">Covered Areas</span>
                          <span className={`text-sm sm:text-base font-bold font-mono block tracking-tight ${isDarkMode ? "text-amber-400" : "text-amber-700"}`}>
                            {bufferResults.impactedDistricts.length} <span className="text-[8px] font-sans text-slate-500 font-normal">Kec.</span>
                          </span>
                        </div>
                     </div>

                     <div className="flex flex-col gap-1 relative z-10">
                        <span className={`text-[8px] font-mono uppercase tracking-widest pl-1 ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>Spasial Terdampak ({bufferResults.impactedVillages.length} Desa)</span>
                        <div className={`max-h-28 overflow-y-auto custom-scrollbar p-1 rounded-xl border ${
                          ...
                          {bufferResults.impactedVillages.slice(0, 10).map((v, idx) => (
                            ...
                            >
                              <span className={`text-[11px] font-semibold transition-colors ${isDarkMode ? "text-slate-300 group-hover:text-white" : "text-slate-700 group-hover:text-slate-950"}`}>{v.name}</span>
                              <span className={`text-[8px] font-mono transition-colors px-1 py-0.5 rounded border ${
                                isDarkMode ? "text-slate-500 group-hover:text-emerald-400 bg-slate-900 border-slate-700" : "text-slate-600 group-hover:text-emerald-700 bg-slate-50 border-slate-200"
                          ...
                          {bufferResults.impactedVillages.length > 10 && (
                            <div className={`text-center text-[9px] py-1.5 italic font-mono rounded-lg mt-1 border border-dashed ${
                              isDarkMode ? "bg-slate-900/40 border-slate-800 text-slate-500" : "bg-slate-50 border-slate-200 text-slate-400"
                            }`}>
                              + {bufferResults.impactedVillages.length - 10} area lainnya
                            </div>
                          )}
                          {bufferResults.impactedVillages.length === 0 && (
                            <div className="text-center text-slate-400 py-3 italic font-sans text-[10px]">Tidak ada pemukiman terdeteksi.</div>
                          )}
                        </div>
                     </div>
                  </div>
                  ...
                  <div className={`flex-1 flex flex-col items-center justify-center border border-dashed rounded-2xl p-5 sm:p-8 min-h-[160px] text-center ${
                    isDarkMode ? "border-slate-700/60 bg-slate-900/10 text-slate-600" : "border-slate-300 bg-slate-50 text-slate-400"
                  }`}>
               ...
               <div className="flex items-center gap-2">
                  <div className="relative">
                    <div className={`h-7 w-7 rounded-full border flex items-center justify-center z-10 relative ${
                      ...
                      <Bot className={`h-3.5 sm:h-4 sm:w-4 ${isDarkMode ? "text-blue-400" : "text-blue-600"}`} />
                    </div>
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className={`text-[8px] sm:text-[9px] font-mono ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>Gemini 3.5-Flash Spatial Engine</span>
                  </div>
               </div>
            </div>
