...
interface PetugasGeraiLoginModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (tenant: MPPTenant, operatorData: { name: string; email: string; role: string }) => void;
  isDarkMode?: boolean;
}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isSuccess) {
        ...
        setSuccessMessage(`Autentikasi berhasil (${authType})! Mengarahkan ke loket ${targetTenant.name}...`);
