...
export const MppServicesMatrixModal: React.FC<MppServicesMatrixModalProps> = ({
  ...
  const categories = [
    { id: 'all', label: 'Semua Layanan (8)', icon: Layers },
    { id: 'dukcapil', label: 'Kependudukan', icon: FileText },
    { id: 'perizinan', label: 'Perizinan Usaha', icon: Zap },
    { id: 'perpajakan', label: 'Pajak & Bapenda', icon: BadgePercent },
    { id: 'sosial', label: 'BPJS & Sosial', icon: HeartHandshake },
    { id: 'kepolisian', label: 'Polres & SIM', icon: ShieldCheck },
    { id: 'disabilitas', label: 'Inklusif Disabilitas', icon: Sparkles },
  ];
        ...
        >
          ...
          <div className="relative pt-3 pb-4 px-5 sm:px-7 border-b border-slate-200/80 dark:border-slate-800 shrink-0 bg-slate-50/80 dark:bg-slate-900/90 backdrop-blur-md">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="hidden sm:inline-flex items-center gap-1 text-[10px] font-bold text-slate-500 dark:text-slate-400">
                      <ShieldCheck className="w-3.5 h-3.5 text-emerald-500" /> Standar Pelayanan Publik Pemkab Luwu
                    </span>
                  </div>
                  <h2 className="text-base sm:text-lg md:text-xl font-extrabold tracking-tight">
                    Katalog & Matriks Layanan Publik MPP
                  </h2>
                </div>
              </div>
            ...
            <div className="relative mt-3">
              ...
              <input
                ...
                placeholder="Cari jenis layanan (contoh: KTP, NIB, PBB, SIM, BPJS, Paspor)..."
            ...
            {filteredServices.length === 0 ? (
              <div className="py-12 text-center space-y-3">
                ...
                <h3 className="text-sm font-bold text-slate-700 dark:text-slate-300">
                  Layanan tidak ditemukan untuk kata kunci "{searchQuery}"
                </h3>
                <button
                  type="button"
                  onClick={() => {
          ...
          <div className="p-4 px-5 sm:px-7 bg-slate-50 dark:bg-slate-800/90 border-t border-slate-200/80 dark:border-slate-800 shrink-0 flex items-center justify-between gap-3">
            <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
              <Zap className="w-3.5 h-3.5 text-emerald-500" /> Semua Layanan Bebas Pungli & Transparan
            </span>
            <button
              type="button"
              onClick={onClose}
