...
import { 
  ...
  UserX, VolumeX, ShieldAlert, AlertTriangle, UserCheck
} from 'lucide-react';
import { supabase } from '../../lib/supabaseClient';
import { MPPTenant, MPPQueue, MPPService, MppReprimand } from '../../types/mpp';
...
export default function TenantDashboard({ isDarkMode, onClose }: Props) {
  ...
  const [operatorInfo, setOperatorInfo] = useState<{ id?: string; userId?: string; name: string; nip?: string; role: string; counterName?: string } | null>(null);
  ...
  // e-Teguran (Real-time Reprimand System)
  const [incomingReprimand, setIncomingReprimand] = useState<MppReprimand | null>(null);
  const [isReprimandHistoryOpen, setIsReprimandHistoryOpen] = useState<boolean>(false);
  const [reprimandCount, setReprimandCount] = useState<number>(0);
  ...
  const fetchDashboardData = useCallback(async (tenantId: string) => {
    ...
    try {
      ...
      if (tData) {
        ...
        setTrackingDocs(tenantTrackings as TrackingDocItem[]);
      }
    } catch (err: any) {
      console.error('Error fetching dashboard data:', err);
      showToast('error', 'Gagal memuat data antrean: ' + (err?.message || 'Error'));
    } finally {
      setIsLoading(false);
    }
  }, []);
            ...
            if (sess && sess.tenantId) {
              ...
              if (matched) {
                ...
                setOperatorInfo({
                  ...
                  userId: sess.userId || sess.id,
                  name: sess.operatorName || sess.name || 'Petugas Loket Gerai',
                  nip: sess.operatorNip || sess.nip || '199001012022011001',
                  role: sess.role || 'petugas_gerai',
                  counterName: sess.counterName || 'Loket 1'
                });
  ...
  const loginAsTenant = (tenant: MPPTenant, opInfo?: { id?: string; userId?: string; name: string; nip?: string; role: string; counterName?: string }) => {
    ...
    const sessionData = {
      ...
      role: operator.role,
      counterName: operator.counterName || 'Loket 1',
    ...
    showToast('success', `Berhasil masuk ke panel ${operator.counterName || 'Loket 1'} (${tenant.name})`);
  };

  const logout = async () => {
    // Clean up active counter session
    try {
      const sessionStr = localStorage.getItem('mpp_tenant_operator_session');
      if (sessionStr) {
        const sess = JSON.parse(sessionStr);
        if (sess) {
          if (sess.email) {
            ...
            delete activeSessions[sess.email.toLowerCase().trim()];
            localStorage.setItem('mpp_active_counter_sessions', JSON.stringify(activeSessions));
          }
          
          // Panggil API backend untuk set is_active = false
          await fetch('/api/mpp/tenant-users/session/end', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              tenant_user_id: sess.tenantUserId || null,
              user_id: sess.userId || null,
              tenant_id: sess.tenantId || null
            })
          });
        }
      }
    } catch (e) {
      console.error('Error clearing active counter session:', e);
    }
  ...
  const handleTestAudio = async () => {
    ...
    try {
      ...
      speakCallingAnnouncement('TEST-01', activeTenant?.name || 'MPP', operatorInfo?.counterName || 'Loket 1');
  ...
  // --- REAL-TIME SUBSCRIPTION FOR QUEUES, SERVICES & REPRIMANDS ---
  useEffect(() => {
    if (!activeTenant) return;

    fetchDashboardData(activeTenant.id);

    // Initial check for any unacknowledged / active 'sent' reprimands
    const checkReprimands = async () => {
      try {
        const reps = await mppService.getReprimands(activeTenant.id);
        
        // Filter reprimands to target specific operator session
        const currentUserId = operatorInfo?.userId || operatorInfo?.id;
        const currentCounterName = operatorInfo?.counterName || 'Loket 1';
        
        const filteredReps = reps.filter((r) => {
          // If a reprimand has target_user_id, it must match currentUserId
          if (r.target_user_id && r.target_user_id !== currentUserId) {
            return false;
          }
          // If a reprimand has target_counter_name, it must match currentCounterName
          if (r.target_counter_name) {
            const rc = r.target_counter_name.toLowerCase().trim();
            const cc = currentCounterName.toLowerCase().trim();
            if (rc !== cc && rc !== `loket ${cc}` && `loket ${rc}` !== cc) {
              return false;
            }
          }
          return true;
        ...
        setReprimandCount(filteredReps.length);
        const sentRep = filteredReps.find((r) => r.status === 'sent');
        if (sentRep) {
          setIncomingReprimand(sentRep);
        } else {
          setIncomingReprimand(null);
        }
      } catch (err) {
        console.error('Error fetching reprimands for tenant:', err);
      }
    };
    checkReprimands();
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'mpp_reprimands',
          filter: `tenant_id=eq.${activeTenant.id}`
        },
        (payload) => {
          if (payload.eventType === 'INSERT') {
            const newRep = payload.new as MppReprimand;
            
            // Check if this new reprimand is targeted to this operator
            const currentUserId = operatorInfo?.userId || operatorInfo?.id;
            const currentCounterName = operatorInfo?.counterName || 'Loket 1';
            
            let isTargeted = true;
            if (newRep.target_user_id && newRep.target_user_id !== currentUserId) {
              isTargeted = false;
            }
            if (newRep.target_counter_name) {
              const rc = newRep.target_counter_name.toLowerCase().trim();
              const cc = currentCounterName.toLowerCase().trim();
              if (rc !== cc && rc !== `loket ${cc}` && `loket ${rc}` !== cc) {
                isTargeted = false;
              }
            ...
            if (isTargeted) {
              checkReprimands();
              if (newRep.status === 'sent') {
                setIncomingReprimand(newRep);
              }
            }
          } else {
            checkReprimands();
  ...
  }, [activeTenant, fetchDashboardData, operatorInfo]);

  // --- QUEUE ACTIONS ---
  const updateQueueStatus = async (queueId: string, newStatus: string) => {
    try {
      const activeCounter = operatorInfo?.counterName || 'Loket 1';
      const activeOfficerId = operatorInfo?.userId || operatorInfo?.id;
      const activeOfficerName = operatorInfo?.name || 'Petugas Loket';

      if (newStatus === 'dipanggil') {
        const callRes = await fetch(`/api/mpp/queues/${queueId}/call`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            counter_name: activeCounter,
            served_by: activeOfficerId,
            served_by_name: activeOfficerName
          })
        });
        const callData = await callRes.json();
        if (!callRes.ok || !callData.success) {
          if (callData.isNoShow) {
            showToast('error', callData.message || 'Batas maksimal pemanggilan 3x tercapai. Pemohon dinyatakan Tidak Hadir (No-Show).');
          } else {
            showToast('error', callData.message || 'Gagal memanggil antrean.');
          }
          if (activeTenant) fetchDashboardData(activeTenant.id);
          return;
        }

        showToast('info', callData.message);
        if (activeTenant) fetchDashboardData(activeTenant.id);
        return;
      }

      const res = await fetch(`/api/mpp/queues/${queueId}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          status: newStatus,
      ...
      if (newStatus === 'masuk_tracking') {
        ...
        showToast('info', `Sedang melayani pemohon di ${activeCounter}.`);
      } else if (newStatus === 'tidak_hadir') {
        showToast('info', 'Antrean ditandai Tidak Hadir (No-Show) dan dikeluarkan dari laporan antrean aktif.');
      }
  ...
  const handleCallNext = async () => {
    ...
    try {
      ...
      // Tarik antrean dengan row-level locking pull mechanism (FOR UPDATE SKIP LOCKED)
      const pulledQueue = await mppService.pullNextQueue({
        tenant_id: activeTenant.id,
        served_by: activeOfficerId,
        served_by_name: activeOfficerName,
        counter_name: activeCounter
      ...
      if (!pulledQueue || !pulledQueue.success || pulledQueue.empty) {
        showToast('info', pulledQueue?.message || 'Tidak ada antrean menunggu berikutnya di gerai ini.');
        return;
      }

      showToast('success', `Berhasil memanggil nomor ${pulledQueue.ticket_code} menuju ${activeCounter}`);
      fetchDashboardData(activeTenant.id);

      try {
        await playAirportChime();
        speakCallingAnnouncement(pulledQueue.ticket_code, activeTenant.name, activeCounter);
      } catch (e) {
        console.error('Audio announcement error:', e);
      }
    } catch (err: any) {
      console.error('Error pulling next queue:', err);
      // Fallback: jika lewat API standar
      if (nextWaitingQueue) {
        await updateQueueStatus(nextWaitingQueue.id, 'dipanggil');
        try {
          await playAirportChime();
          speakCallingAnnouncement(nextWaitingQueue.ticket_code, activeTenant.name, activeCounter);
        } catch (e) {
          console.error(e);
        }
      } else {
        showToast('error', 'Gagal memanggil antrean berikutnya: ' + (err?.message || 'Error'));
      }
    }
  };
    ...
    <div className="fixed inset-0 z-[100] flex flex-col md:flex-row bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans">
      ...
      <div className={`hidden md:flex w-full md:w-72 border-r flex flex-col shrink-0 ${isDarkMode ? 'border-slate-800 bg-slate-900' : 'border-slate-200 bg-white'}`}>
        <div className="p-6 border-b border-inherit">
          ...
          <div className="mt-4 p-3.5 rounded-2xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/60">
            <div className="flex items-center justify-between">
              <div className="text-[11px] uppercase tracking-wider font-bold text-slate-400">Gerai Terpilih</div>
              <span className="text-[10px] font-mono font-black px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-600 dark:text-amber-400 border border-amber-500/30">
                {operatorInfo?.counterName || 'Loket 1'}
              </span>
            </div>
            <div className="font-black text-base leading-tight mt-1 text-slate-800 dark:text-slate-100">{activeTenant.name}</div>
            ...
            {operatorInfo?.name && (
              <div className="mt-2 pt-2 border-t border-slate-200/60 dark:border-slate-700/50 flex items-center gap-1.5 text-[11px] text-slate-600 dark:text-slate-300">
                <UserCheck className="w-3.5 h-3.5 text-indigo-500 shrink-0" />
                <span className="truncate font-semibold">{operatorInfo.name}</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <div className={`hidden md:flex h-16 border-b items-center justify-between px-6 shrink-0 ${isDarkMode ? 'border-slate-800 bg-slate-900' : 'border-slate-200 bg-white'}`}>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => setIsReprimandHistoryOpen(true)}
              className="px-3 py-1.5 rounded-xl text-xs font-bold bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/30 flex items-center gap-1.5 transition-all cursor-pointer shadow-sm"
              title="Lihat Riwayat & Status e-Teguran Resmi dari Kepala DPMPTSP"
            >
            ...
            <MppFoHelpTriggerButton
              sourceType="gerai_tenant"
              sourceName={`Gerai ${activeTenant.name}`}
              variant="emergency"
            />
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              {currentlyActiveQueue ? (
                <div className="p-4 sm:p-6 rounded-3xl border-2 border-indigo-500/40 bg-gradient-to-br from-indigo-500/10 via-indigo-500/5 to-transparent shadow-xl relative overflow-hidden">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-2.5 pt-3 border-t border-indigo-500/20">
                    {/* 1. Panggil Ulang */}
                    {((currentlyActiveQueue.call_count || 1) >= 3 && currentlyActiveQueue.status === 'dipanggil') ? (
                      <button
                        type="button"
                        disabled
                        className="h-13 px-3 rounded-2xl bg-slate-200 dark:bg-slate-800 text-slate-400 font-bold text-xs flex items-center justify-center gap-1.5 cursor-not-allowed opacity-60"
                        title="Batas maksimal pemanggilan (3x) telah tercapai"
                      >
                        <VolumeX className="w-4 h-4 text-rose-500" /> Max 3x Terlampaui
                      </button>
                    ) : (
                      <button
                        type="button"
                        onClick={async () => {
                          ...
                          await updateQueueStatus(currentlyActiveQueue.id, 'dipanggil');
                          playAirportChime().then(() => {
                            speakCallingAnnouncement(
                              currentlyActiveQueue.ticket_code,
                              activeTenant.name,
                              operatorInfo?.counterName || 'Loket 1'
                            );
                      ...
                      >
                        <Volume2 className="w-4 h-4 animate-pulse" /> Panggil ({currentlyActiveQueue.call_count || 1}/3)
                      </button>
                    )}
                    ...
                    {/* 5. Tandai Tidak Hadir (No-Show) */}
                    <button
                      type="button"
                      onClick={() => {
                        if (navigator.vibrate) navigator.vibrate([100]);
                        updateQueueStatus(currentlyActiveQueue.id, 'tidak_hadir');
                      }}
                      className="h-13 px-3 rounded-2xl bg-rose-600 hover:bg-rose-700 active:scale-95 text-white font-bold text-xs flex items-center justify-center gap-1.5 shadow-sm transition-all cursor-pointer"
                      title="Tandai pemohon tidak hadir setelah dipanggil"
                    >
                      ...
                      <div className="flex flex-wrap gap-2 border-t pt-3.5 border-inherit">
                        {queue.status === 'menunggu' && (
                          <button 
                            onClick={() => {
                              ...
                              playAirportChime().then(() => {
                                speakCallingAnnouncement(queue.ticket_code, activeTenant?.name || 'MPP', operatorInfo?.counterName || 'Loket 1');
                              });
                            }} 
                        ...
                        {queue.status === 'dipanggil' && (
                          <div className="flex-1 flex flex-wrap gap-2">
                            {(queue.call_count || 1) >= 3 ? (
                              <button 
                                disabled
                                className="h-12 px-3 bg-slate-200 dark:bg-slate-800 text-slate-400 rounded-2xl font-bold flex items-center justify-center gap-1.5 text-xs cursor-not-allowed opacity-60"
                                title="Batas maksimal 3x pemanggilan telah tercapai"
                              >
                                <VolumeX className="w-4 h-4 text-rose-500" /> Max 3x
                              </button>
                            ) : (
                              <button 
                                onClick={async () => {
                                  if (navigator.vibrate) navigator.vibrate([100, 50, 100]);
                                  await updateQueueStatus(queue.id, 'dipanggil');
                                  playAirportChime().then(() => {
                                    speakCallingAnnouncement(queue.ticket_code, activeTenant?.name || 'MPP', operatorInfo?.counterName || 'Loket 1');
                                  });
                                }} 
                                className="h-12 px-3 bg-amber-500 hover:bg-amber-600 active:scale-95 text-white rounded-2xl font-bold flex items-center justify-center gap-1.5 text-xs shadow-sm cursor-pointer transition-all"
                                title="Panggil Ulang"
                              >
                                <Volume2 className="w-4 h-4 animate-pulse" /> Panggil ({queue.call_count || 1}/3)
                              </button>
                            )}
                            ...
                            <button 
                              onClick={() => {
                                if (navigator.vibrate) navigator.vibrate([100]);
                                updateQueueStatus(queue.id, 'tidak_hadir');
                              }} 
                              className="h-12 px-2.5 bg-rose-600 hover:bg-rose-700 active:scale-95 text-white rounded-2xl font-bold flex items-center justify-center gap-1 text-xs shadow-sm cursor-pointer transition-all"
                              title="Tandai pemohon Tidak Hadir (No-Show)"
                            >
                              <UserX className="w-4 h-4" /> Tidak Hadir
                            </button>
                          </div>
      ...
      {/* ========================================================================= */}
      {/* e-TEGURAN: HARD-STOP SAFETY MODAL (PETUGAS WAJIB KONFIRMASI) */}
      {/* ========================================================================= */}
      {activeTenant && (
        <MppReprimandHardStopModal
          reprimand={incomingReprimand}
          tenantName={activeTenant.name}
          onAcknowledged={(updated) => {
            setIncomingReprimand(null);
            showToast('success', 'Konfirmasi penerimaan e-Teguran berhasil dikirim ke Kepala DPMPTSP.');
          }}
        />
      )}

      {/* ========================================================================= */}
      {/* MODAL: RIWAYAT e-TEGURAN GERAI */}
      {/* ========================================================================= */}
      {activeTenant && (
        <ReprimandModal
          isOpen={isReprimandHistoryOpen}
          onClose={() => setIsReprimandHistoryOpen(false)}
          tenants={tenants}
          selectedTenantId={activeTenant.id}
          filterByUserId={operatorInfo?.userId || operatorInfo?.id}
