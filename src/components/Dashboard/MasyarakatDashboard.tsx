...
export default function MasyarakatDashboard({
  ...
  const [nama, setNama] = useState(() => {
    return activeProfile?.full_name || (typeof window !== "undefined" ? localStorage.getItem("luwu_user_name") || "" : "");
  });
  ...
  const fetchAndHydrateMasyarakatProfile = async () => {
    try {
      ...
      // Primary Query: Fetch matching profile row using user.id
      ...
      // Secondary Query Fallback 1: Query by Email
      ...
      // Secondary Query Fallback 2: Check NIK from email format or localStorage (e.g. 7317130202700002@warga.luwukab.go.id)
      const cleanNikFromEmail = (effectiveEmail.includes("@warga.luwukab.go.id") || !effectiveEmail.includes("@"))
        ? effectiveEmail.split("@")[0].trim()
        : "";
      const storedNik = localStorage.getItem("luwu_user_nik") || "";
      const potentialNik = prof?.nik || activeProfile?.nik || (/^\d{16}$/.test(cleanNikFromEmail) ? cleanNikFromEmail : (/^\d{16}$/.test(storedNik) ? storedNik : ""));

      if (!prof && potentialNik && /^\d{16}$/.test(potentialNik)) {
        try {
          const res = await fetch(`/api/profiles?nik=${encodeURIComponent(potentialNik)}`, { credentials: 'same-origin' });
          if (res.ok) {
            const resJson = await res.json();
            if (resJson.data) prof = resJson.data;
          }
        ...
        // Secondary Query Fallback 3: Lookup mpp_citizens (Single Source of Truth for Citizen/MPP Data)
        if (!prof) {
          try {
        ...
        // Fallback from localStorage luwu_citizen_data if available
        if (!prof) {
          try {
            const rawCit = localStorage.getItem("luwu_citizen_data");
            if (rawCit) {
              const cit = JSON.parse(rawCit);
              if (cit && (cit.nik || cit.full_name)) {
                prof = {
      ...
      let resolvedNama = rawNama;
      if (resolvedNama && ["masyarakat", "masyarakat publik", "offline-user", "masyarakat pemohon"].includes(resolvedNama.toLowerCase())) {
        if (prof?.full_name && !["masyarakat", "masyarakat publik", "offline-user"].includes(prof.full_name.toLowerCase())) {
          resolvedNama = prof.full_name;
        } else {
          resolvedNama = "";
        }
      }
    ...
    try {
      ...
      try {
        ...
        await supabase.from("gis_pkkpr").insert({
          ...
          nib_oss: pkkprNib || null,
          nama_badan_usaha: pkkprCategory === "Berusaha" ? (pkkprPerusahaan || "Pelaku Usaha") : null,
          nama_pemohon: finalNama,
  ...
  const handleGetLocation = () => {
    ...
    const fallbackToKecamatan = () => {
      ...
      Swal.fire({
        ...
        text: `Menggunakan pusat koordinat Kecamatan ${targetText || "Luwu"} sebagai referensi lokasi.`,
        icon: "info",
        toast: true,
        position: "top-end",
        timer: 4000,
    ...
    <div className={`min-h-screen ${isDarkMode ? "bg-slate-950 text-slate-100" : "bg-slate-50 text-slate-900"} font-sans pb-12 transition-all duration-300`}>
      ...
      {mainArea === "survey_skm" && (
        <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 mt-6 relative z-10 animate-in fade-in duration-300">
          <MppOtpVerificationGuard
            userType="masyarakat"
            defaultNik={userNik || activeProfile?.nik || ""}
            defaultPhone={kontak || extractPhone(activeProfile)}
            defaultName={nama || activeProfile?.full_name || ""}
            featureTitle="Survei Kepuasan Masyarakat (SKM)"
            featureSubtitle="Sesuai PermenPAN-RB, pengisian Indeks Kepuasan Masyarakat (IKM) wajib divalidasi dengan NIK 16-Digit dan verifikasi kode OTP WhatsApp aktif untuk menjamin keabsahan penilaian layanan."
      ...
      {mainArea === "testimoni" && (
        <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 mt-6 relative z-10 animate-in fade-in duration-300">
          <MppOtpVerificationGuard
            ...
            defaultPhone={kontak || extractPhone(activeProfile)}
            defaultName={nama || activeProfile?.full_name || ""}
            featureTitle="Testimoni & Ulasan Pengguna"
            featureSubtitle="Untuk mencegah penyebaran ulasan palsu atau spam, penulisan testimoni pelayanan MPP Simpurusiang wajib diverifikasi terlebih dahulu melalui NIK dan kode OTP WhatsApp aktif."
            isDarkMode={isDarkMode}
          >
