...
export default function PertanianLandClearanceDashboard() {
  ...
  const fetchAgrarianQueue = async () => {
    ...
    try {
      ...
      if (error) {
        console.warn('Supabase fetch error for Pertanian queue:', error);
      }

      if (data && data.length > 0) {
        const mapped: AgrarianQueueItem[] = data.map((item: any, idx: number) => {
          const isLp2bIntersect = idx % 2 === 0;
          ...
          const resolvedDistrictName = matchedDist 
            ? matchedDist.name 
            : (rawDist ? rawDist.replace(/^dist_/i, '').replace(/_/g, ' ') : (idx % 2 === 0 ? 'Bua Ponrang' : 'Suli'));
          const resolvedDistrictId = matchedDist ? matchedDist.id : (rawDist || (idx % 2 === 0 ? 'dist_bua_ponrang' : 'dist_suli'));

          const rawVil = item.village_id || item.villageId || item.desa || item.id_desa || '';
          const resolvedVillageName = rawVil 
            ? rawVil.replace(/^vil_/i, '').replace(/_/g, ' ') 
            : (idx % 2 === 0 ? 'Tampumia' : 'Lawi-Lawi');

          return {
            id: item.id || `AGRI-${Math.random().toString(36).substring(2, 7)}`,
            ...
            companyName: item.name || 'PT Agro Luwu Sejahtera',
            sector: item.sector || 'Pertanian & Pengolahan Hasil Bumi',
            districtId: resolvedDistrictId,
            districtName: resolvedDistrictName,
            villageId: item.village_id || item.villageId || undefined,
            villageName: resolvedVillageName,
            areaHa: item.area_ha || 12.4,
            existingCrop: isLp2bIntersect ? 'Sawah Irigasi Teknis (Padi IP300)' : 'Perkebunan Kakao & Palawija',
            puptrForwardedNotes: item.override_justification || 
              (isLp2bIntersect
                ? `DIREKOMENDASIKAN DARI PUPTR: Poligon investasi beririsan dengan kawasan LP2B Sawah Irigasi di Kec. ${resolvedDistrictName} seluas ${(item.area_ha ? item.area_ha * 0.35 : 4.2).toFixed(1)} Ha. Memerlukan Verifikasi Lapangan & Berita Acara Alih Fungsi dari Dinas Pertanian.`
                : `PUPTR Forward: Usulan lokasi di Kec. ${resolvedDistrictName} berdekatan dengan saluran irigasi sekunder. Mohon analisis produktivitas tanah & rekomendasi dinas.`),
            agriStatus: item.berita_acara_num ? 'Approved' : (item.status === 'Rejected' ? 'Rejected' : 'Pending Review'),
            ...
            replacementLandHa: item.replacement_land_ha || (item.area_ha ? item.area_ha * 1.0 : 12.4),
        ...
        if (!selectedApp && mapped.length > 0) {
          selectAppForReview(mapped[0]);
        }
      } else {
        // Honest fallback: empty array if database has no active clearance records
        setQueueList([]);
        setSelectedApp(null);
      }
    } catch (err) {
  ...
  const handleSubmitApproval = async () => {
    ...
    try {
      ...
      if (error) {
        console.warn('Supabase update error for Agrarian approval:', error);
      }
  ...
  const handleSubmitRejection = async () => {
    ...
    try {
      ...
      if (error) {
        console.warn('Supabase update error for Agrarian rejection:', error);
      }
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm space-y-4">
        ...
        {isLoadingQueue ? (
          ...
          <div className="py-8 text-center text-slate-500 font-medium rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/50 text-xs">
            Belum ada antrean permohonan yang memerlukan rekomendasi Dinas Pertanian.
          </div>
        ) : (
          <>
        ...
        {showDocumentPreview && selectedApp && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-950/85 backdrop-blur-md animate-in fade-in duration-200 overflow-y-auto print:p-0 print:bg-white print:static">
            ...
            >
              ...
              <div id="pertanian-bap-printable-document" className="space-y-6 bg-white p-2 rounded-xl">
                {/* ══════════════════════════════════════════════════════════
              ...
              <div className="bap-page-1 border border-slate-200 p-6 sm:p-8 rounded-2xl bg-white space-y-6 print:border-none print:p-0">
                ...
                <div className="flex items-center gap-4 border-b-4 border-double border-slate-900 pb-4">
                  ...
                  <div className="flex-1 text-center space-y-0.5">
                    ...
                    <p className="text-[10px] text-slate-600 leading-tight">
                      Kompleks Perkantoran Pemkab Luwu, Jl. Jend. Sudirman No. 01 Belopa, Sulawesi Selatan
                    </p>
                    <p className="text-[9.5px] text-slate-500 font-mono">
                      Email: pertanian@luwukab.go.id &bull; Website: https://pertanian.luwukab.go.id &bull; Kode Pos: 91994
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 text-center text-xs pt-6 font-serif">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="font-bold text-slate-900 uppercase">Kepala Dinas Pertanian Kab. Luwu,</p>
                    ...
                    <p className="font-bold underline text-slate-950">IR. H. JUMADI, M.Si.</p>
                    <p className="text-[10px] text-slate-500 font-mono">NIP. 19710324 199603 1 002</p>
                  </div>
                </div>
              </div>
