...
export default function InvestorPortalDashboard() {
  ...
  const fetchAndHydrateCorporateProfile = async () => {
    try {
      ...
      const profileData = {
        namaPenanggungJawab: pNama || companyName || "Investor Corporate",
        namaPerusahaan: pPerusahaan || companyName || "PT Investor Utama",
        nib: pNib || "9120000000000",
        emailPerusahaan: pEmail || "investor@luwukab.go.id"
      };
  ...
  useEffect(() => {
    const loadUserSession = async () => {
      ...
      try {
        ...
        const { data: profile } = await supabase.from('profiles').select('role').eq('id', effectiveId).maybeSingle();
        ...
        if (!profile && effectiveId) {
          try {
            await supabase.from('profiles').upsert({
              id: effectiveId,
              role: effectiveRole,
              full_name: session.user.user_metadata?.company_name || session.user.user_metadata?.full_name || 'Investor Terverifikasi'
            });
          } catch (e) {}
        }
        ...
        {isLoading ? (
          ...
          <div className="py-4 sm:py-8 animate-in fade-in zoom-in-95 duration-300">
            <MppOtpVerificationGuard
              userType="investor"
              defaultName={companyName || "Investor Luwu"}
              featureTitle="Survei Kepuasan Investor (SKM MPP)"
              featureSubtitle="Sebagai investor terdaftar di Kabupaten Luwu, partisipasi Anda dalam Survei Kepuasan Masyarakat (PermenPAN-RB) memerlukan validasi NIK penanggung jawab dan OTP WhatsApp aktif guna memastikan keabsahan data evaluasi investasi daerah."
              isDarkMode={true}
            >
          ...
          <div className="py-4 sm:py-8 animate-in fade-in zoom-in-95 duration-300">
            <MppOtpVerificationGuard
              userType="investor"
              defaultName={companyName || "Investor Luwu"}
              featureTitle="Testimoni & Review Investasi Daerah"
              featureSubtitle="Untuk mencegah manipulasi reputasi iklim investasi, ulasan dan testimoni investor wajib tervalidasi menggunakan NIK Penanggung Jawab dan kode OTP WhatsApp aktif."
              isDarkMode={true}
            >
