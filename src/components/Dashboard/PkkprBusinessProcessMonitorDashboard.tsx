...
export default function PkkprBusinessProcessMonitorDashboard() {
  ...
  // Fetch data from Supabase gis_pkkpr and investments tables (Doktrin Zero Dummy & Honest Fallback)
  const fetchSyncData = async () => {
    setIsLoading(true);
    try {
      let combined: PkkprSyncItem[] = [];
      const now = new Date().getTime();

      // 1. Fetch from gis_pkkpr (Primary dynamic table for submissions)
      try {
        ...
        if (!gisErr && gisData && gisData.length > 0) {
          gisData.forEach((item: any) => {
            ...
            if (item.sk_pkkpr_num || item.pertek_puptr_num || item.status_pkkpr === 'Approved' || item.status_pkkpr === 'Published') {
              puptrStat = 'APPROVED';
            } else if (item.status_pkkpr === 'Requires Revision' || item.status_pkkpr === 'Rejected') {
              puptrStat = 'REJECTED';
            } else if (item.status_pkkpr === 'Review' || item.status_pkkpr === 'Forwarded_To_Pertanian' || item.status_pkkpr === 'Approved_Pertanian') {
              puptrStat = 'IN_REVIEW';
            }
            ...
            combined.push({
              ...
              villageName: item.desa_kelurahan || '-',
              ...
              puptrDocNumber: item.pertek_puptr_num || (puptrStat === 'APPROVED' ? `503/PERTEK-PUPTR/LUWU/${item.id.split('-').pop()}` : undefined),
              puptrNotes: item.catatan_teknis || 'Verifikasi tata ruang sesuai dengan ketentuan RTRW.',
              puptrUpdatedDate: item.updated_at,
              pertanianStatus: pertStat,
              pertanianDocNumber: undefined,
              pertanianBaNumber: item.berita_acara_pertanian_num || undefined,
              pertanianNotes: pertStat === 'APPROVED' ? 'Rekomendasi teknis alih fungsi disetujui dengan Berita Acara.' : undefined,
        ...
        console.warn('Error fetching gis_pkkpr for monitor:', err);
      }
      ...
      try {
          ...
          invData.forEach((item: any) => {
            if (!combined.some(c => c.id === item.id)) {
              ...
              combined.push({
                ...
                nibNik: item.plot_number || item.nib || '-',
                applicantName: item.contact_pic || item.applicant_name || item.name || 'Pemohon PKKPR',
                companyName: item.company_name || item.name || 'Pelaku Usaha',
                ...
                puptrUpdatedDate: item.updated_at,
                pertanianStatus: pertStat,
                pertanianDocNumber: item.surat_rekomendasi_num || undefined,
                pertanianBaNumber: item.berita_acara_num || undefined,
                pertanianNotes: pertStat === 'APPROVED' ? 'Rekomendasi teknis alih fungsi disetujui.' : undefined,
                pertanianUpdatedDate: item.updated_at,
                printingStatus: printStat,
        ...
        console.warn('Error fetching investments for monitor:', err);
      }
