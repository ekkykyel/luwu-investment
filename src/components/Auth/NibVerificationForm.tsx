  ...
  return (
    <div className={`w-full max-w-md mx-auto p-6 rounded-2xl border ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} shadow-xl`}>
      <div className="flex items-center gap-3 mb-6">
        <div className={`p-2.5 rounded-lg ${isDarkMode ? 'bg-blue-500/20 text-blue-400' : 'bg-blue-100 text-blue-600'}`}>
          <ShieldCheck size={24} />
        </div>
      ...
      <AnimatePresence mode="wait">
        {!isSuccess ? (
          ...
          >
            <div>
              <label className={`block text-xs font-medium mb-1.5 uppercase tracking-wider ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                ...
                <div className="flex flex-col items-center gap-2 text-slate-400 pointer-events-none">
                  {file ? (
                    <>
                      ...
                      <span className={`text-xs font-medium truncate max-w-[200px] ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                        {file.name}
                      </span>
                    </>
                  ) : (
