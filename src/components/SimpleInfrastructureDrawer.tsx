    ...
    <div className="fixed inset-0 z-[200] flex flex-col md:flex-row bg-slate-900 shadow-2xl">
      ...
      <div className={`w-full md:w-96 flex flex-col max-h-full border-b md:border-b-0 md:border-r ${isDarkMode ? "bg-slate-950 border-white/10 text-white" : "bg-white border-slate-200 text-slate-800"}`}>
        ...
        <div className="flex-1 p-5 overflow-y-auto custom-scrollbar flex flex-col gap-4">
          ...
          <div className={`p-4 rounded-xl border flex flex-col gap-1.5 transition-all ${coordinates ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-400" : "bg-indigo-500/5 border-indigo-500/20 text-slate-400"}`}>
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider">
              <MapPin className={`h-4 w-4 ${coordinates ? "text-emerald-400" : "text-indigo-400 animate-pulse"}`} />
              <span>Titik Koordinat Lokasi</span>
            </div>
            {coordinates ? (
              <div className="font-mono text-xs flex flex-col gap-0.5 text-slate-700 dark:text-slate-300">
                <span>Longitude: <strong className="text-emerald-400">{coordinates[0].toFixed(6)}</strong></span>
                <span>Latitude: <strong className="text-emerald-400">{coordinates[1].toFixed(6)}</strong></span>
              </div>
            ) : (
          ...
          {/* Name Input */}
          <div>
            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Nama Aset Infrastruktur</label>
            <input 
              type="text" 
              value={name}
          ...
          <div>
            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Keterangan Singkat / Informasi Pendukung</label>
            <textarea 
              value={description}
        ...
        <div className="p-5 border-t border-inherit flex gap-2">
          <button 
            onClick={onClose}
            disabled={isSaving}
            className={`flex-1 py-3 text-xs font-bold uppercase rounded-xl border transition cursor-pointer ${isDarkMode ? "border-slate-800 text-slate-400 hover:bg-slate-900 hover:text-white" : "border-slate-300 text-slate-500 hover:bg-slate-50 hover:text-slate-700"}`}
          >
            Batal
          </button>
      ...
      <div className="flex-1 relative bg-slate-950" style={{ height: "70vh" }}>
        ...
        <div className={`absolute bottom-6 right-6 w-64 max-h-[60vh] overflow-y-auto custom-scrollbar p-4 flex flex-col gap-3 rounded-xl shadow-xl backdrop-blur-md border transition-all duration-300 z-[200] ${
          ...
          <div className="flex items-center justify-between border-b pb-2 mb-1 border-slate-500/20">
            ...
            <Sparkles className={`h-4 w-4 ${isDarkMode ? "text-indigo-400" : "text-indigo-600"}`} />
          </div>
                ...
                <label key={idx} className="flex items-center justify-between cursor-pointer group">
                  <div className="flex items-center gap-2">
                    <span className={`p-1.5 rounded-lg border transition-colors ${
                      isActive 
                        ? (isDarkMode ? "bg-indigo-500/20 border-indigo-500/30 text-indigo-400" : "bg-indigo-50 border-indigo-200 text-indigo-600")
                        : (isDarkMode ? "bg-slate-800/50 border-slate-700 text-slate-500" : "bg-slate-100 border-slate-200 text-slate-400")
                    }`}>
        ...
        <div className="absolute top-4 left-6 right-6 md:left-1/2 md:right-auto md:-translate-x-1/2 pointer-events-none bg-slate-900/90 backdrop-blur border border-white/10 px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-3 animate-in slide-in-from-top-4 duration-300">
          ...
          <p className="text-xs text-slate-300 font-sans tracking-wide">
            Silakan <strong className="text-indigo-400 font-bold">Klik ikon Point di kiri atas peta</strong> lalu tempatkan pin lokasi infrastruktur baru Anda.
          </p>
        </div>
      </div>
    </div>
