        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left text-xs whitespace-nowrap">
            <thead className="bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-700/50">
              <tr>
                <th className="px-4 py-3 font-bold text-slate-800 dark:text-slate-200">Waktu (Ping)</th>
                <th className="px-4 py-3 font-bold text-slate-800 dark:text-slate-200">Tipe Event</th>
                <th className="px-4 py-3 font-bold text-slate-800 dark:text-slate-200">Keterangan</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 bg-white dark:bg-slate-900/20">
              {logs.length === 0 ? (
                <tr>
                  <td colSpan={3} className="px-4 py-6 text-center text-slate-600 dark:text-slate-400 italic text-[10px]">
                ...
                logs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5 text-slate-800 dark:text-slate-200 text-[10px] font-mono">
