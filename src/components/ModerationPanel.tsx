    ...
    <div className="p-4 flex-1 flex flex-col min-h-0">
      <div className="flex items-center justify-between mb-4 border-b border-slate-800 pb-2">
        ...
        <button
          ...
          className="p-1 ml-2 text-rose-400 hover:text-white rounded hover:bg-rose-900 transition-colors border border-rose-900/50"
      ...
      {loading ? (
        <div className="flex-1 flex flex-col items-center justify-center py-10">
          ...
          <p className="text-xs text-slate-500">Memuat data draf...</p>
        </div>
      ) : error ? (
        <div className="p-3 bg-red-600/10 border border-red-500/20 text-red-400 rounded-xl flex items-start gap-2 text-xs">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center p-4 border-2 border-dashed border-slate-800 rounded-xl bg-slate-900/10">
          ...
          <p className="text-[11px] text-slate-500">Keren! Tidak ada data spasial berstatus Draf / usulan baru yang menunggu persetujuan.</p>
        </div>
      ) : (
          ...
          {drafts.map((item) => (
            ...
            >
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-[10px] text-slate-400 flex items-center gap-1.5 mt-0.5">
                    {item.collection === "infrastruktur" ? (
                      <span className="flex items-center gap-0.5 text-indigo-400 font-sans font-semibold">
                        <MapPin className="h-3 w-3" /> Infra
                      </span>
                    ) : (
                      <span className="flex items-center gap-0.5 text-emerald-400 font-sans font-semibold">
                        🗺️ Potensi
                      </span>
                    )}
              ...
              {item.raw?.properties?.district && (
                <div className="text-[10px] text-slate-500 font-sans italic bg-slate-950/20 p-1.5 rounded">
