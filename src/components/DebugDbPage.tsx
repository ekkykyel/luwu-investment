...
export default function DebugDbPage() {
  ...
  // Run server-side test via Express API
  const testServerConnection = async () => {
    ...
    try {
      ...
      setServerResult({
        success: false,
        error: `Fetch Exception: ${err.message || String(err)}`
      });
    } finally {
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans p-6 md:p-12 flex flex-col gap-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 flex flex-col gap-4">
          ...
          <div className="space-y-3 mt-2">
            <div className="flex items-center justify-between p-3 rounded-xl bg-slate-950/40 border border-slate-800/60">
              <span className="text-xs text-slate-400">Vite Configured URL</span>
              {urlConfigured ? (
                <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                  <CheckCircle2 className="h-3 w-3" /> Configured
                </span>
              ) : (
                <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase bg-rose-500/10 text-rose-400 border border-rose-500/20">
            ...
            <div className="flex items-center justify-between p-3 rounded-xl bg-slate-950/40 border border-slate-800/60">
              <span className="text-xs text-slate-400">Anon Key Configured</span>
              {anonKeyConfigured ? (
                <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                  <CheckCircle2 className="h-3 w-3" /> Configured
                </span>
            ...
            <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800 text-[11px] font-mono leading-relaxed space-y-1 text-slate-400">
          ...
          <div className="flex-1 flex flex-col gap-3 mt-2">
            <div className="flex items-center justify-between p-3 rounded-xl bg-slate-950/40 border border-slate-800/60">
              ...
              {clientTesting ? (
                ...
                <span className="flex items-center gap-1 px-2.5 py-0.5 rounded bg-emerald-500/10 text-emerald-400 text-xs font-bold uppercase border border-emerald-500/20">
                  Success
                </span>
              ) : clientResult ? (
                <span className="flex items-center gap-1 px-2.5 py-0.5 rounded bg-rose-500/10 text-rose-400 text-xs font-bold uppercase border border-rose-500/20">
                  Failed
                </span>
              ) : (
            ...
            <div className="flex items-center justify-between p-3 rounded-xl bg-slate-950/40 border border-slate-800/60">
              <span className="text-xs text-slate-400">Latency Check</span>
            ...
            <div className="flex-1 flex flex-col min-h-[140px]">
              <span className="text-xs font-semibold text-slate-400 mb-1 block">Raw Response / Error Object:</span>
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 flex flex-col gap-4">
          ...
          <div className="flex-1 flex flex-col gap-3 mt-2">
            ...
            <div className="flex items-center justify-between p-3 rounded-xl bg-slate-950/40 border border-slate-800/60">
              <span className="text-xs text-slate-400">Express Endpoint</span>
              <span className="text-xs text-indigo-400 font-mono font-semibold bg-indigo-500/10 px-2 py-0.5 rounded border border-indigo-500/20">
            ...
            <div className="flex-1 flex flex-col min-h-[140px]">
              <span className="text-xs font-semibold text-slate-400 mb-1 block">Raw Server Response:</span>
              <pre className="flex-1 bg-slate-950 p-3 rounded-xl border border-slate-800 text-[10px] font-mono overflow-auto max-h-[180px] leading-relaxed text-slate-300">
                {serverTesting ? (
                  "Requesting status from serverless backend..."
                ) : serverResult ? (
      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
              ...
              <div>
                ...
                <div className="mt-2 text-xs font-mono">
                  Status: {dbStatusTesting ? "Checking..." : dbStatusResult?.success ? (
                    <span className="text-emerald-400 font-bold">READY / SYNCED</span>
                  ) : (
                    <span className="text-amber-400 font-bold">NOT FULLY HYDRATED / SYNC FAILED</span>
                  )}
                </div>
              </div>
            </div>
          ...
          <div>
            <span className="text-xs font-semibold text-slate-400 mb-1 block">Raw Sync Status Payload (/api/db-status):</span>
