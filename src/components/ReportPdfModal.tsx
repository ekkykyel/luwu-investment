...
import { District, Investment, SektorInvestasi, Village } from "../types.js";
...
export default function ReportPdfModal({
  ...
  const captureMap = async () => {
    ...
    try {
      // Tunggu sebentar agar peta stabil
      ...
      if (!imgData && mapCanvasWrapper) {
        try {
          ...
          const mCanvas = await pdfRenderQueue.enqueue(() => safeHtml2Canvas(mapCanvasWrapper as HTMLElement, {
            scale: 2.5, // 2.5x resolution for print-ready high density cartography
            useCORS: true,
            allowTaint: true,
            logging: false,
  ...
  const getLogisticsProximity = (inv: Investment) => {
    ...
    if (pgRouting !== undefined && pgRouting !== null) {
      return {
        ...
        distToRoadKm: gisData?.jarak_bandara || parseFloat((Number(pgRouting) * 0.4).toFixed(1)),
        isPgRouting: true,
    ...
    if (gisData && (gisData.jarak_pelabuhan !== undefined || gisData.jarak_bandara !== undefined)) {
      return {
        ...
        distToRoadKm: gisData.jarak_bandara ? parseFloat((gisData.jarak_bandara * 0.4).toFixed(1)) : 0,
        isPgRouting: false,
        source: "GIS Legacy"
      };
    }
    ...
    // Euclidean Fallback
    const latDiffPort = Math.abs(inv.latitude - (-3.3860616));
    const lngDiffPort = Math.abs(inv.longitude - 120.3979346);
    const distToPort = Math.sqrt(latDiffPort * latDiffPort + lngDiffPort * lngDiffPort) * 111; 

    const latDiffAirport = Math.abs(inv.latitude - (-3.0863384));
    const lngDiffAirport = Math.abs(inv.longitude - 120.2413232);
    const distToAirport = Math.sqrt(latDiffAirport * latDiffAirport + lngDiffAirport * lngDiffAirport) * 111; 

    const distToNationalRoad = Math.abs(inv.longitude - 120.322) * 111;
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      const canvas = await pdfRenderQueue.enqueue(() => safeHtml2Canvas(element, {
        scale: 4, // 4-fold scale yields crisp vector and typography PDF
        useCORS: true,
        allowTaint: true,
        logging: false,
        backgroundColor: "#ffffff",
      ...
      const filename = reportType === "district"
        ? `Laporan_Investasi_Kecamatan_${currentDistrict.name.replace(/\s+/g, "_")}.pdf`
        : `Peluang_Investasi_Proyek_${currentInvestment.name.replace(/\s+/g, "_")}.pdf`;
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-[9999] flex items-center justify-center p-4">
      <motion.div initial={{ scale: 0.95, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }} transition={{ type: "spring", stiffness: 300, damping: 30 }} className="bg-slate-100 rounded-3xl w-full max-w-5xl h-[95vh] shadow-2xl border border-slate-200 overflow-hidden flex flex-col">
        ...
        <div className="bg-slate-950 text-white px-6 py-4 flex justify-between items-center border-b border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-xs font-mono font-bold tracking-widest text-emerald-400">PDF GENERATOR ENGINE</h3>
              <span className="text-sm font-display font-bold">Laporan Kinerja Spasial & Investasi Luwu</span>
            </div>
          </div>
          <button 
            onClick={onClose}
            className="p-1.5 hover:bg-slate-900 text-slate-400 hover:text-white rounded-xl transition-all h-8 w-8 flex items-center justify-center cursor-pointer"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
          ...
          <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto justify-start sm:justify-end">
            {reportType === "district" ? (
              <div className="flex items-center gap-1.5 text-xs text-slate-700">
                <span className="font-semibold text-slate-500 font-mono text-[10px] hidden sm:inline">PILIH KECAMATAN:</span>
                <select
              ...
              <div className="flex items-center gap-1.5 text-xs text-slate-700">
                <span className="font-semibold text-slate-500 font-mono text-[10px] hidden sm:inline">PILIH PROYEK:</span>
                <select
            ...
            <div className="flex items-center gap-2 sm:border-l sm:border-slate-300 sm:pl-3">
              <span className="font-semibold text-slate-500 font-mono text-[10px] hidden sm:inline">BASEMAP:</span>
              <select
                value={mapMode}
            ...
            {isCapturingMap ? (
              ...
              <button
                type="button"
                onClick={captureMap}
                className="flex items-center gap-1 px-3 py-1.5 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-xl text-[10px] font-bold text-slate-700 transition-all cursor-pointer whitespace-nowrap"
          ...
          >
            ...
            <div className="border-b-4 border-double border-slate-900 pb-4 flex items-center justify-between gap-4">
              <div className="flex items-center justify-center shrink-0">
                <img src="https://i.ibb.co.com/KxKKb5d8/transparant.png" style={{ width: "40px", height: "40px" }} className="w-10 h-10 object-contain" alt="Logo Luwu" crossOrigin="anonymous" />
              </div>
              <div className="flex-1 text-center font-serif">
                ...
                <p className="text-[9px] font-sans text-slate-500 italic mt-1 font-medium">
              ...
              <div className="flex flex-col gap-6 flex-1">
                ...
                <div className="grid grid-cols-4 gap-4 bg-slate-50 border border-slate-200/80 p-4 rounded-xl shadow-sm">
                  <div className="flex flex-col gap-1.5 border-r border-slate-200 pr-2">
                    <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider font-mono">Luas Wilayah</span>
                    ...
                    <span className="text-[9px] text-slate-400 leading-tight">Total Area Kecamatan</span>
                  </div>
                  <div className="flex flex-col gap-1.5 border-r border-slate-200 px-2">
                    <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider font-mono">Kerapatan Penduduk</span>
                    ...
                    <span className="text-[9px] text-slate-400 leading-tight">Populasi: {formatNumber(currentDistrict.population)}</span>
                  </div>
                  <div className="flex flex-col gap-1.5 border-r border-slate-200 px-2">
                    <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider font-mono">Nilai Investasi</span>
                    <span className="text-sm font-extrabold font-mono text-emerald-600">{formatRupiah(districtTotalInvestmentValue)}</span>
                    <span className="text-[9px] text-slate-400 leading-tight">{districtInvestments.length} Proyek Aktif Terdaftar</span>
                  </div>
                  <div className="flex flex-col gap-1.5 pl-2">
                    <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider font-mono">Skor Infrastruktur</span>
                    ...
                    <span className="text-[9px] text-slate-400 leading-tight">Kelayakan logistik</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6 mt-2">
                  ...
                  <div className="border border-slate-200/80 p-4 rounded-xl flex flex-col gap-3">
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider font-mono block">Distribusi Sektor Masuk (Miliar IDR)</span>
                    
                    {isSectorDataEmpty ? (
                      <div className="h-48 flex items-center justify-center border border-dashed border-slate-200 rounded-lg text-xs italic text-slate-400 p-4 text-center">
                  ...
                  <div className="border border-slate-200/80 p-4 rounded-xl flex flex-col gap-3">
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider font-mono block">Komparasi Nilai Investasi Antar Kecamatan (Miliar)</span>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="border border-slate-200/80 p-3 rounded-xl flex flex-col gap-2 bg-slate-50 relative overflow-hidden">
                    <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider font-mono block">Citra Navigasi Kartografis Map</span>
                    <div className="h-32 border border-slate-200 rounded-lg bg-white overflow-hidden relative flex items-center justify-center">
                      {mapSnapshotUrl ? (
                        ...
                        <div className="flex flex-col items-center justify-center p-4 text-center text-[10px] text-slate-400">
                  ...
                  <div className="border border-slate-200/80 p-3 rounded-xl flex flex-col gap-2 bg-slate-50 relative overflow-hidden">
                    <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider font-mono block">Blueprint Teknis Wilayah</span>
                    <div className="h-32 border border-slate-200 rounded-lg bg-white relative flex items-center justify-center p-2 select-none">
                      <svg viewBox="0 0 400 110" className="w-full h-full text-slate-400 opacity-85">
                        ...
                        <text x="202" y="52" fontSize="7" fontFamily="monospace" fill="#ef4444">CENTER [{currentDistrict.coordinates[0].toFixed(3)}°, {currentDistrict.coordinates[1].toFixed(3)}°]</text>
                ...
                <div className="flex flex-col gap-2 mt-auto">
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider font-mono block">Daftar Proyek Investasi Terdaftar di Wilayah Ini ({districtInvestments.length})</span>
                    ...
                    <table className="w-full text-left text-[10px] border-collapse bg-white">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {districtInvestments.length === 0 ? (
                          <tr>
                            <td colSpan={5} className="p-4 text-center italic text-slate-400 bg-slate-50/50">
                          ...
                          districtInvestments.slice(0, 5).map((inv, idx) => (
                            <tr key={`${inv.id}-${idx}`} className={idx % 2 === 0 ? "bg-white" : "bg-slate-50/30"}>
                              ...
                              <td className="p-2 text-slate-500 font-mono truncate max-w-[120px]">{inv.contactPic} ({inv.phoneNumber})</td>
                            </tr>
                          ))
                        )}
                      </tbody>
                ...
                <div className="border-t border-slate-100 pt-6 flex justify-between items-end mt-auto text-[10px] text-slate-400 font-mono">
                  <div>
              ...
              <div className="flex flex-col gap-6 flex-1">
                ...
                <div className="bg-slate-950 text-white p-5 rounded-2xl shadow-xl flex gap-6 relative overflow-hidden items-center border border-slate-800">
                  ...
                  <div className="flex-1 flex flex-col gap-1 hover:brightness-110 transition-all font-sans">
                    <span className="text-[9px] text-emerald-400 font-bold uppercase tracking-wider font-mono">Peluang Nilai Pembiayaan</span>
                    ...
                    <p className="text-[10px] text-slate-400 font-mono mt-1 leading-relaxed">
                      Kebutuhan Lahan Kerja: <span className="text-amber-400 font-bold">{currentInvestment.areaHa} Hektar</span> dengan sertifikasi <span className="font-semibold text-white">{currentInvestment.landStatus}</span>.
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="border border-slate-200/80 p-4 rounded-xl flex flex-col gap-3">
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider font-mono block">Letak Koordinat & Administrasi Wilayah</span>
                    ...
                    <div className="flex flex-col gap-2 text-[10.5px]">
                      ...
                      <div className="flex justify-between border-b border-slate-100 pb-1.5">
                        <span className="text-slate-s550 font-sans">Latitude (Garis Lintang):</span>
                        <span className="font-bold text-slate-900 font-mono">{currentInvestment.latitude.toFixed(6)}° S</span>
                      </div>
                      <div className="flex justify-between border-b border-slate-100 pb-1.5">
                        <span className="text-slate-550 font-sans">Longitude (Garisbujur):</span>
                        <span className="font-bold text-slate-900 font-mono">{currentInvestment.longitude.toFixed(6)}° E</span>
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-50 border border-slate-200/60 p-3 rounded-lg flex flex-col gap-1.5">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-[10px]">
                        <div>
                          <span className="text-slate-400 block text-[8.5px] uppercase font-mono leading-none">Ke Bandara Bua</span>
                          <span className="font-bold font-mono text-indigo-700">{(currentInvestmentLogistics as any).distToAirportKm} KM</span>
                        </div>
                        <div>
                          <span className="text-slate-400 block text-[8.5px] uppercase font-mono leading-none">Ke Pelabuhan Ulo-Ulo</span>
                          <span className="font-bold font-mono text-indigo-700">{currentInvestmentLogistics.distToPortKm} KM</span>
                        </div>
                        <div className="col-span-2 pt-1 border-t border-slate-200">
                          <span className="text-slate-400 block text-[8.5px] uppercase font-mono leading-none">Akses Arterial Nasional</span>
                          <span className="font-bold font-mono text-indigo-700">{currentInvestmentLogistics.distToRoadKm} KM</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="border border-slate-200/80 p-4 rounded-xl flex flex-col gap-3">
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider font-mono block">Proporsi Valuasi Terhadap Rata-Rata Sektor</span>
                    
                    <div className="h-32 w-full flex items-center justify-center select-none mt-2">
                      <BarChart width={290} height={140} data={[
                        { name: "Proyek Ini", value: Math.round(currentInvestment.investmentValue / 1e9) },
                        { name: `Rata-rata ${currentInvestment.sector}`, value: Math.round(averageValueInSector / 1e9) }
                    ...
                    <p className="text-[10px] text-slate-500 leading-normal text-justify">
                      Proyek ini memiliki nilai taksiran investasi sebesar <span className="font-semibold text-slate-900">{formatRupiah(currentInvestment.investmentValue)}</span>, dibandingkan dengan angka mean sektoral regional yang berada di tingkat <span className="font-semibold text-slate-900">{formatRupiah(averageValueInSector)}</span>.
                    </p>
                    ...
                    {currentInvestment.smartData?.financials?.[0] && (
                      <div className="bg-emerald-50/50 border border-emerald-100 p-3 rounded-lg flex flex-col gap-1.5 mt-2">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-[10px]">
                          <div>
                            <span className="text-slate-400 block text-[8.5px] uppercase font-mono leading-none">Proyeksi ROI</span>
                            <span className="font-bold font-mono text-emerald-700">{currentInvestment.smartData.financials[0].roi || 0}%</span>
                          </div>
                          <div>
                            <span className="text-slate-400 block text-[8.5px] uppercase font-mono leading-none">NPV Estimasi</span>
                          ...
                          <div>
                            <span className="text-slate-400 block text-[8.5px] uppercase font-mono leading-none">IRR</span>
                            <span className="font-bold font-mono text-emerald-700">{currentInvestment.smartData.financials[0].irr || 0}%</span>
                          </div>
                          <div>
                            <span className="text-slate-400 block text-[8.5px] uppercase font-mono leading-none">Payback Period</span>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {/* Left Column: Map Snapshot */}
                  <div className="border border-slate-200/80 p-3 rounded-xl flex flex-col gap-2 bg-slate-50 relative overflow-hidden">
                    <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider font-mono block">Plot Lokasi Geografis rill</span>
                    <div className="h-32 border border-slate-200 rounded-lg bg-white overflow-hidden relative flex items-center justify-center">
                      {mapSnapshotUrl ? (
                        <img 
                          src={mapSnapshotUrl} 
                  ...
                  {/* Right Column: Blueprint schematic */}
                  <div className="border border-slate-200/80 p-3 rounded-xl flex flex-col gap-2 bg-slate-50 relative overflow-hidden">
                    <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider font-mono block">Situs Plot Spasial Node</span>
                    <div className="h-32 border border-slate-200 rounded-lg bg-white relative flex items-center justify-center p-2 select-none font-sans">
                      <svg viewBox="0 0 400 110" className="w-full h-full text-slate-400 opacity-85">
                        ...
                        <text x="215" y="63" fontSize="7" fontFamily="sans-serif" fill="#1e293b">{currentInvestment.name.toUpperCase()}</text>
                        
                        <text x="5" y="15" fontSize="6.5" fontFamily="monospace" fill="#64748b">LINTANG LUAT: {currentInvestment.latitude.toFixed(5)}° S</text>
                        <text x="5" y="25" fontSize="6.5" fontFamily="monospace" fill="#64748b">BUJUR TIMUR: {currentInvestment.longitude.toFixed(5)}° E</text>
                      </svg>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col gap-0.5">
                        <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider font-mono">Dinas Penanggungjawab / Kepala PIC</span>
                        <span className="font-semibold text-slate-800 font-sans">{currentInvestment.contactPic}</span>
                        <span className="text-[9px] text-slate-500">DPMPTSP Bidang Hubungan Luar & CSR</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col gap-0.5">
                        <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider font-mono">Telepon Direktori</span>
