...
import { formatJarakMeter } from "../lib/formatters";
...
export default function SpatialQueryPanel({ 
  ...
  return (
    <div className={`shrink-0 border p-4 sm:p-5 rounded-2xl flex flex-col gap-4 font-sans relative overflow-hidden transition-all duration-300 w-full h-full ${isDarkMode ? "bg-slate-900/60 border-slate-700/50 shadow-[0_4px_24px_-8px_rgba(0,0,0,0.5)] text-slate-100" : "bg-white/80 border-slate-200/60 shadow-lg text-slate-800"}`}>
      ...
      <div className={`flex items-center gap-3 relative z-10 border-b pb-3 ${isDarkMode ? "border-white/5" : "border-slate-200"}`}>
        <div className={`p-1.5 rounded-lg ${isDarkMode ? "bg-emerald-500/10" : "bg-emerald-50 border border-emerald-100"}`}>
          <Compass className={`h-4.5 w-4.5 rotate-45 ${isDarkMode ? "text-emerald-700 dark:text-emerald-400" : "text-emerald-600"}`} />
        </div>
      ...
      {/* Feature Banner: Land Suitability & Soil Quality Filter */}
      {onOpenSuitabilityModal && (
        <button
          type="button"
          onClick={onOpenSuitabilityModal}
          className="p-3 rounded-2xl bg-gradient-to-r from-emerald-600/20 via-emerald-500/10 to-teal-500/20 border border-emerald-500/40 text-white hover:border-emerald-400 transition-all shadow-md group text-left cursor-pointer relative z-10"
        >
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <span className="text-[11px] font-bold text-emerald-400 block font-mono">
                  Filter Spasial Kesesuaian Lahan & Tanah
                </span>
                <span className="text-[9.5px] text-slate-300 block">
                  Analisis kualitas tanah FAO untuk komoditas Kakao, Kopi, Udang, Padi & Mineral
                </span>
              </div>
            </div>
      ...
      <div className="flex flex-col gap-2 pt-1 relative z-10">
        <span className={`text-[8.5px] font-extrabold uppercase tracking-widest font-mono ${isDarkMode ? "text-emerald-700 dark:text-emerald-400/85" : "text-emerald-700"}`}>{t('spatial.instantSpatialSearch')}</span>
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className={`p-2 border rounded-xl text-left text-[10px] transition-all font-semibold flex flex-col gap-0.5 shadow-sm hover:scale-[1.01] active:scale-[0.99] ${
              isDarkMode 
                ? "border-slate-800 bg-slate-900/50 hover:border-emerald-500 hover:bg-emerald-950/35 text-slate-600 dark:text-slate-400" 
                : "border-slate-300 bg-slate-50 hover:border-emerald-500 hover:bg-emerald-50 text-slate-600 dark:text-slate-400"
            }`}
          >
          ...
          <button
            ...
            className={`p-2 border rounded-xl text-left text-[10px] transition-all font-semibold flex flex-col gap-0.5 shadow-sm hover:scale-[1.01] active:scale-[0.99] ${
              isDarkMode 
                ? "border-slate-800 bg-slate-900/50 hover:border-blue-500 hover:bg-blue-950/35 text-slate-600 dark:text-slate-400" 
                : "border-slate-300 bg-slate-50 hover:border-blue-500 hover:bg-blue-50 text-slate-600 dark:text-slate-400"
            }`}
          >
          ...
          <button
            ...
            className={`p-2 border rounded-xl text-left text-[10px] transition-all font-semibold flex flex-col gap-0.5 shadow-sm hover:scale-[1.01] active:scale-[0.99] ${
              isDarkMode 
                ? "border-slate-800 bg-slate-900/50 hover:border-green-500 hover:bg-green-950/35 text-slate-600 dark:text-slate-400" 
                : "border-slate-300 bg-slate-50 hover:border-green-500 hover:bg-green-50 text-slate-600 dark:text-slate-400"
            }`}
          >
          ...
          <button
            ...
            className={`p-2 border rounded-xl text-left text-[10px] transition-all font-semibold flex flex-col gap-0.5 shadow-sm hover:scale-[1.01] active:scale-[0.99] ${
              isDarkMode 
                ? "border-slate-800 bg-slate-900/50 hover:border-rose-500 hover:bg-rose-950/35 text-slate-600 dark:text-slate-400" 
                : "border-slate-300 bg-slate-50 hover:border-rose-500 hover:bg-rose-50 text-slate-600 dark:text-slate-400"
            }`}
          >
      ...
      <form onSubmit={handleQuerySubmit} className={`flex flex-col gap-4 border-t pt-4 text-xs font-sans relative z-10 ${isDarkMode ? "border-white/5" : "border-slate-200"}`}>
        <span className={`text-[8.5px] font-extrabold uppercase tracking-widest font-mono ${isDarkMode ? "text-emerald-700 dark:text-emerald-400/85" : "text-emerald-700"}`}>{t('spatial.customFilter')}</span>
        
        {/* Sliders distance and values */}
        <div className="flex flex-col gap-3.5 mt-1">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={`text-[8px] sm:text-[9px] block font-extrabold mb-1 uppercase font-mono tracking-widest ${isDarkMode ? "text-slate-600 dark:text-slate-400" : "text-slate-600 dark:text-slate-400"}`}>{t('spatial.minLand')}</label>
              <input
                type="number"
                min="0"
                value={minArea}
            ...
            <div>
              <label className={`text-[8px] sm:text-[9px] block font-extrabold mb-1 uppercase font-mono tracking-widest ${isDarkMode ? "text-slate-600 dark:text-slate-400" : "text-slate-600 dark:text-slate-400"}`}>{t('spatial.minVal')}</label>
              <input
                type="number"
                min="0"
                value={minValue}
          ...
          <div className={`p-2.5 rounded-xl border ${isDarkMode ? "bg-slate-900/40 border-slate-800" : "bg-slate-100/50 border-slate-200"}`}>
            <div className="flex justify-between items-center text-[9px] mb-2">
              <span className={`font-extrabold uppercase font-mono tracking-widest ${isDarkMode ? "text-slate-600 dark:text-slate-400" : "text-slate-800 dark:text-slate-200"}`}>{t('spatial.maxDistBua')}</span>
          ...
          <div className={`p-2.5 rounded-xl border ${isDarkMode ? "bg-slate-900/40 border-slate-800" : "bg-slate-100/50 border-slate-200"}`}>
            <div className="flex justify-between items-center text-[9px] mb-2">
              <span className={`font-extrabold uppercase font-mono tracking-widest ${isDarkMode ? "text-slate-600 dark:text-slate-400" : "text-slate-800 dark:text-slate-200"}`}>{t('spatial.maxDistRoad')}</span>
              <span className={`font-mono font-bold px-1.5 py-0.5 rounded shadow-inner text-[9.5px] ${
                isDarkMode ? "bg-slate-800 border border-slate-700 text-slate-300" : "bg-white border border-slate-300 text-slate-800"
          ...
          <div className={`p-3 rounded-2xl border flex flex-col gap-3 relative overflow-hidden transition-all duration-300 ${
            ...
            <div className="flex items-center gap-2">
              <div className={`p-1 rounded-lg ${isDarkMode ? "bg-emerald-500/10" : "bg-emerald-50"}`}>
                <MapPin className={`h-3.5 w-3.5 ${isDarkMode ? "text-emerald-700 dark:text-emerald-400" : "text-emerald-600"}`} />
              </div>
              <span className={`text-[10px] font-extrabold uppercase tracking-widest font-mono ${isDarkMode ? "text-emerald-700 dark:text-emerald-400/90" : "text-emerald-700"}`}>Proximity Radius Filter</span>
            </div>

            {/* Target Infrastructure Dropdown & Quick Facility Type Selector */}
            <div className="flex flex-col gap-1.5">
              <div className="flex justify-between items-center">
                <label className={`text-[8.5px] font-extrabold uppercase font-mono tracking-widest ${isDarkMode ? "text-slate-600 dark:text-slate-400" : "text-slate-600"}`}>Target Infrastructure</label>
            ...
            {selectedInfraId && (
              <div className="flex flex-col gap-2 mt-1 transition-all duration-350">
                <div className="flex justify-between items-center text-[8.5px] font-mono font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider">
                    ...
                    <button
                      ...
                      className={`py-1.5 px-1 border rounded-lg text-center transition-all flex flex-col items-center justify-center gap-0.5 ${
                        proximityRadius === preset.radius
                          ...
                          : isDarkMode
                          ? "bg-slate-950 border-slate-800 text-slate-600 dark:text-slate-400 hover:border-emerald-500/50 hover:text-slate-200"
                ...
                <div className={`text-[10px] font-medium leading-relaxed mt-1 ${isDarkMode ? "text-slate-300" : "text-slate-800 dark:text-slate-200"}`}>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[9px] font-mono font-bold text-slate-600 dark:text-slate-400">1 KM</span>
                  <input
                    type="range"
                    min="1"
                    max="30"
                  ...
                  />
                  <span className="text-[9px] font-mono font-bold text-slate-600 dark:text-slate-400">30 KM</span>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="mt-1">
          <label className={`text-[8px] sm:text-[9px] block font-extrabold mb-2 uppercase font-mono tracking-widest ${isDarkMode ? "text-slate-600 dark:text-slate-400" : "text-slate-505"}`}>{t('spatial.targetIndustrySector')}</label>
                ...
                <button
                  ...
                  className={`px-2.5 py-1.5 rounded-lg text-[8.5px] font-sans font-bold transition-all border uppercase tracking-wider ${
                    active
                      ...
                      : (isDarkMode ? "bg-slate-950/50 hover:bg-slate-900 text-slate-600 dark:text-slate-400 border-slate-800/85" : "bg-white hover:bg-slate-50 text-slate-600 dark:text-slate-400 border-slate-300")
                  }`}
                >
      ...
      <div className={`border-t pt-4 flex flex-col gap-3 relative z-10 ${isDarkMode ? "border-white/5" : "border-slate-200"}`}>
          ...
          <div className="p-3.5 rounded-2xl bg-slate-900/95 border border-emerald-500/50 shadow-[0_10px_25px_rgba(16,185,129,0.15)] text-white flex flex-col gap-2 relative overflow-hidden">
            <div className="flex justify-between items-center border-b border-emerald-500/20 pb-2">
              <div className="flex items-center gap-1.5">
                <Route className="w-4 h-4 text-emerald-700 dark:text-emerald-400 animate-pulse" />
              ...
              {onClearShortestPathRoute && (
                <button
                  ...
                  className="p-1 rounded-md bg-slate-800 hover:bg-slate-700 text-slate-600 dark:text-slate-400 hover:text-white transition-colors"
                  title="Tutup Rute"
                >
            ...
            <div className="flex flex-col gap-1 text-[10.5px]">
              <div className="flex justify-between items-center text-slate-300">
                <span className="text-slate-600 dark:text-slate-400 font-mono text-[9.5px]">Awal:</span>
                <span className="font-bold text-emerald-300 truncate max-w-[180px]">{activeRouteInfo.infraName}</span>
              </div>
              <div className="flex justify-between items-center text-slate-300">
                <span className="text-slate-600 dark:text-slate-400 font-mono text-[9.5px]">Tujuan:</span>
            ...
            <div className="mt-1 pt-2 border-t border-slate-800 flex justify-between items-center">
              <div>
                <span className="text-[8.5px] font-mono text-slate-600 dark:text-slate-400 block uppercase">Metode Topologi</span>
                <span className="text-[9.5px] font-bold text-emerald-700 dark:text-emerald-400 font-mono flex items-center gap-1">
                  <Sparkles className="w-3 h-3 text-emerald-700 dark:text-emerald-400" />
                  {activeRouteInfo.method}
                </span>
              </div>
              <div className="text-right">
                <span className="text-[8.5px] font-mono text-slate-600 dark:text-slate-400 block uppercase">Jarak Presisi</span>
                <span className="text-xs font-black font-mono text-emerald-700 dark:text-emerald-400 bg-emerald-500/20 px-2 py-0.5 rounded border border-emerald-500/30">
                  {formatJarakMeter(activeRouteInfo.distanceMeters)}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="flex justify-between items-center text-[10px]">
          <span className={`font-extrabold uppercase tracking-widest font-mono ${isDarkMode ? "text-emerald-700 dark:text-emerald-400/85" : "text-emerald-700"}`}>{t('spatial.areaMatch')} ({queryResults.length})</span>
          {queryResults.length > 0 && <span className="text-slate-600 dark:text-slate-400 font-bold font-mono">{t('spatial.sortedByScore')}</span>}
        </div>

        {queryResults.length === 0 ? (
          <div className={`p-4 rounded-xl border border-dashed text-center text-[10px] font-mono italic ${
            isDarkMode ? "border-slate-750 bg-slate-900/30 text-slate-600 dark:text-slate-400" : "border-slate-300 bg-slate-50 text-slate-600 dark:text-slate-400"
          }`}>
            Belum ada data infrastruktur dalam radius {proximityRadius} KM
          </div>
        ) : (
              ...
              >
                ...
                {selectedInfraId && onSelectShortestPathRoute && (
                  <button
                    ...
                    className="w-full mt-1 py-1.5 px-2.5 rounded-lg bg-emerald-600/20 hover:bg-emerald-600/30 border border-emerald-500/40 text-emerald-700 dark:text-emerald-400 font-mono text-[9.5px] font-bold transition-all flex items-center justify-center gap-1.5 hover:border-emerald-400"
                  >
                    <Route className="w-3.5 h-3.5 text-emerald-700 dark:text-emerald-400 animate-pulse" />
...
// ui polish: expand spatial query panel to fix asymmetrical layout
