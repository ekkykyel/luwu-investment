...
import { GeoJSONLayer, Investment, SektorInvestasi } from "../types.js";
...
export default function MapLegend({
  ...
  const handleCopyGeoJSON = (layer: GeoJSONLayer) => {
    try {
      navigator.clipboard.writeText(JSON.stringify(layer.geojson, null, 2));
  ...
  return (
    <>
      <div 
        id="map-legend-panel"
        className={`absolute bottom-[90px] md:bottom-[24px] left-4 md:left-[352px] z-[40] w-[290px] sm:w-[325px] max-w-[330px] transition-all duration-300 ease-in-out font-sans backdrop-blur-md border border-white/40 dark:border-slate-700/50 border-t-[3px] shadow-2xl rounded-2xl ${
          isDarkMode 
      ...
      <div 
        ...
        className="flex items-center justify-between px-4 py-3.5 border-b border-slate-200/60 dark:border-white/10 cursor-pointer selection:bg-transparent transition-all hover:bg-black/5 dark:hover:bg-white/5"
      >
        <div className="flex items-center gap-2.5">
          <LuwuLogo size="sm" />
          <div className="flex flex-col">
            <span className={`text-[12px] sm:text-xs font-black uppercase tracking-widest font-display leading-tight ${isDarkMode ? "text-white" : "text-slate-900"}`}>
              Legenda Spasial Peta
            </span>
            <span className={`text-[9px] sm:text-[10px] font-mono leading-tight tracking-wider ${isDarkMode ? "text-emerald-400 font-bold" : "text-indigo-605 font-bold"}`}>
              Kabupaten Luwu
            </span>
          </div>
        </div>
          ...
          <div className={`flex px-3 pt-3 pb-2 shrink-0 border-b gap-1.5 transition-colors ${
            ...
            <button
              ...
              className={`flex-1 py-1.5 rounded-xl text-[11px] font-black font-sans transition-all duration-200 flex items-center justify-center gap-1.5 border cursor-pointer select-none ${
                legendTab === "aktif"
                  ? isDarkMode
                    ? "bg-emerald-500/15 border-emerald-500/35 text-emerald-400 shadow-md shadow-emerald-500/5"
                    : "bg-indigo-50 border-indigo-250 text-indigo-700 shadow-sm"
                  : isDarkMode
                    ? "bg-transparent border-transparent text-slate-400 hover:text-slate-200 hover:bg-white/5"
                    : "bg-transparent border-transparent text-slate-500 hover:text-slate-850 hover:bg-black/5"
              }`}
            >
            ...
            <div className={`px-4 pt-4 pb-2 border-b shrink-0 transition-colors ${
              ...
              <div className={`flex items-center border rounded-lg px-2.5 py-2 focus-within:border-emerald-500 dark:focus-within:border-emerald-400 transition-colors ${
                ...
                <input
                  ...
                  className={`bg-transparent border-none outline-none text-xs w-full ml-2 font-mono placeholder:text-slate-400 font-medium ${
              ...
              <>
                ...
                {choroplethMetric !== "none" && (
                  <div className="flex flex-col gap-2">
                    <span className={`text-[10px] font-black uppercase tracking-widest font-mono flex items-center gap-1.5 ${isDarkMode ? "text-slate-400" : "text-indigo-950"}`}>
                      ...
                      <div className="flex flex-col gap-1.5">
                        ...
                        <div className="flex">
                          <span className={`text-[9px] font-bold font-mono px-2 py-0.5 rounded border uppercase tracking-wider ${
                            isDarkMode ? "text-slate-400 bg-slate-900 border-white/5" : "text-slate-700 bg-white border-slate-250 shadow-xs"
                          }`}>
                          ...
                          return items.map((item, idx) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <span className={isVisible ? (isDarkMode ? "text-slate-200 font-medium" : "text-slate-800 font-bold") : (isDarkMode ? "text-slate-400 font-medium" : "text-slate-500 font-semibold")}>
                                  {item.label}
                                </span>
                              </div>
                            );
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className={`p-3.5 rounded-xl border flex flex-col gap-2.5 ${
                      ...
                      <div className="flex flex-col gap-1 text-xs">
                        <span className={`font-bold tracking-wide ${isDarkMode ? "text-amber-400" : "text-amber-700"}`}>
                          Aura Kepadatan:
                        </span>
                    ...
                    <button
                      ...
                      className={`text-[9px] font-mono px-2 py-0.5 rounded-md transition-all select-none border whitespace-nowrap cursor-pointer font-bold ${
                        isInteractiveMode 
                          ...
                          : isDarkMode 
                            ? "bg-slate-950/85 border-white/10 text-slate-500 hover:bg-slate-950" 
                  ...
                  <div className={`p-3.5 rounded-xl border flex flex-col gap-3 ${
                        ...
                        return (
                          <div
                            ...
                            className={`flex items-center gap-2 p-1.5 rounded-lg border transition-all ${
                              ...
                              isVisibleOnMap 
                                ...
                                : isDarkMode 
                                  ? "border-transparent text-slate-500 opacity-40 grayscale-[40%]" 
                                  : "border-transparent text-slate-400 opacity-40 grayscale-[40%]"
                            } ${
                              isInteractiveMode && isVisibleOnMap
                                ? isDarkMode 
                                  ? "bg-emerald-500/15 border-emerald-500/30 text-emerald-400 ring-1 ring-emerald-500/25" 
                          ...
                          >
                            ...
                            <span className="text-[10px] sm:text-[11px] leading-tight font-semibold flex-1 truncate">
                              {sec.textName} 
                              <span className={`text-[8.5px] ml-1 font-bold ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>
                                ({visibleCount})
                              </span>
                            </span>
                          </div>
                    ...
                    {isInteractiveMode && (
                      <div className={`text-[9px] border-t pt-2 mt-1 leading-tight flex items-center justify-center gap-1.5 ${
                        isDarkMode ? "text-slate-400 border-white/5" : "text-slate-550 border-slate-200"
                      }`}>
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between col-span-2">
                      ...
                      <div className="flex items-center gap-1.5 select-none">
                        <span className={`text-[9px] font-mono font-bold ${isDarkMode ? "text-slate-500" : "text-slate-600"}`}>Urut:</span>
                        <select
                          value={layerSortBy}
                    ...
                    <div className={`p-3 rounded-xl border flex flex-col gap-3.5 max-h-[250px] overflow-y-auto custom-scrollbar ${
                      ...
                      {displayedSpatialLayers.length === 0 ? (
                        <div className="text-center py-5 text-[10px] text-slate-500 font-mono">
                          ...
                          <div key={category} className={`flex flex-col gap-2 border rounded-lg p-2.5 shrink-0 ${
                            ...
                            >
                              <span className={`text-[10px] font-black uppercase tracking-wider ${
                                isDarkMode ? "text-slate-400 group-hover:text-slate-205" : "text-slate-700 group-hover:text-slate-950 font-bold"
                              }`}>{category} ({layersInCategory.length})</span>
                              {collapsedCategories[category] ? (
                                <ChevronDown className="h-3.5 w-3.5 text-slate-500 group-hover:text-slate-400" />
                              ) : (
                                <ChevronUp className="h-3.5 w-3.5 text-slate-500 group-hover:text-slate-400" />
                              )}
                            </div>
                                  ...
                                  <div className="flex items-center justify-between gap-2.5 min-w-0 pointer-events-auto">
                                    <div className="flex items-center gap-1.5 min-w-0 flex-1">
                                      {layerSearchQuery.length === 0 && (
                                        <div 
                                          className="text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 cursor-grab active:cursor-grabbing p-0.5 shrink-0 flex items-center justify-center"
                                      ...
                                      <div className="flex flex-col min-w-0 flex-1">
                                        <div className="flex items-center gap-1.5 min-w-0">
                                          ...
                                          <button
                                            ...
                                            className={`p-1 rounded-md transition-all cursor-pointer inline-flex items-center justify-center shrink-0 focus:outline-none ${
                                              isDarkMode ? "hover:bg-white/10 text-slate-400 hover:text-blue-400" : "hover:bg-slate-200 text-slate-500 hover:text-blue-600"
                                            }`}
                                        ...
                                        <span className={`text-[8.5px] uppercase font-mono tracking-wide leading-none mt-1 font-extrabold ${isDarkMode ? "text-slate-500" : "text-slate-500"}`}>
                                          {l.category}
                                        </span>
                                      </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-0.5 shrink-0">
                                      <button
                                        ...
                                        className={`p-1.5 rounded-md transition-all cursor-pointer shrink-0 ${
                                          isDarkMode ? "hover:bg-white/10 text-slate-300" : "hover:bg-slate-200 text-slate-700 shadow-xs"
                                        } ${
                                          l.isActive 
                                            ? isDarkMode ? "text-emerald-400" : "text-emerald-600" 
                                            : "text-slate-450"
                                        }`}
                                        title={l.isActive ? "Sembunyikan layer" : "Tampilkan layer"}
                                      >
                                        {l.isActive ? (
                                    ...
                                    <div className={`flex flex-col gap-1.5 border-t pt-2 mt-1.5 p-2 rounded-xl border ${
                                      ...
                                      <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-1">
                                          <Sliders size={11} className={isDarkMode ? "text-emerald-400" : "text-emerald-600"} />
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className={`p-3.5 rounded-xl border grid grid-cols-2 gap-2.5 text-[11px] font-mono ${
                    isDarkMode ? "bg-slate-950/60 border-slate-800/80 text-slate-300" : "bg-slate-50 border-slate-205 text-slate-700 shadow-sm"
                  }`}>
                ...
                <div className={`flex items-center gap-2 text-[10px] p-2.5 rounded-xl border font-mono leading-relaxed text-center justify-center ${
                  isDarkMode ? "text-slate-350 bg-slate-950 border-slate-800" : "text-slate-700 bg-slate-100 border-slate-200 shadow-xs"
                }`}>
                  <HelpCircle className="h-3.5 w-3.5 text-blue-500 shrink-0" />
                  <span className="font-semibold">Klik pada wilayah kecamatan, batas desa, atau pin sektoral untuk detail informasi spasial kawan.</span>
                </div>
              </>
            ) : (
              <>
                ...
                {(() => {
                  ...
                  if (!isAnyLayerActive) {
                    return (
                      <div className="flex flex-col items-center justify-center py-10 text-center px-4 animate-in fade-in duration-200">
                        ...
                        <p className={`text-[10px] leading-relaxed font-mono mt-1.5 max-w-[220px] mx-auto ${
                          isDarkMode ? "text-slate-500" : "text-slate-500"
                        }`}>
                          Silakan masuk ke tab <strong className="text-emerald-400 font-extrabold uppercase">"Kelola Layer"</strong> untuk mengaktifkan layer spasial pada peta.
                        </p>
                      </div>
                    );
                  }
                  ...
                  return (
                    <div className="flex flex-col gap-4 animate-in fade-in duration-200">
                      <div className={`text-[10px] font-mono leading-none tracking-wide pb-1 border-b flex items-center justify-between ${
                        isDarkMode ? "text-slate-400 border-white/5" : "text-slate-500 border-slate-250"
                      }`}>
                        <span>REPRESENTASI DATA AKTIF</span>
                        <span className={`font-bold px-1.5 py-0.5 rounded ${isDarkMode ? "text-emerald-400 bg-emerald-500/10" : "text-indigo-700 bg-indigo-50"}`}>
                            ...
                            {(() => {
                              ...
                              return items.map((item, idx) => (
                                <div key={idx} className="flex items-center gap-2">
                                  ...
                                  <span className={isDarkMode ? "text-slate-300 font-medium" : "text-slate-700 font-semibold"}>{item.label}</span>
                                </div>
                              ));
                            })()}
                          </div>
                        ...
                        <div className={`p-3 rounded-xl border flex flex-col gap-2 transition-all ${
                          ...
                          <div className="flex items-start gap-2.5 min-w-0">
                            ...
                            <div className="flex flex-col min-w-0">
                              ...
                              <span className="text-[9px] font-mono text-amber-400 font-extrabold uppercase mt-0.5">
                                Kepadatan: {
                          ...
                          <p className={`text-[10px] leading-relaxed font-mono ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}>
                      ...
                      {activeCustomLayers.map((l) => {
                        ...
                        return (
                          <div key={l.id} className={`p-3 rounded-xl border flex flex-col gap-2.5 transition-all ${
                              ...
                              <div className="flex flex-col min-w-0 flex-1">
                                ...
                                <div className="flex flex-wrap items-center gap-1.5 mt-0.5 text-[8.5px] font-mono leading-none">
                                  <span className={`px-1.5 py-0.5 rounded uppercase font-black tracking-wider ${
                                    isDarkMode ? "bg-slate-900 text-slate-400 border border-white/5" : "bg-slate-100 text-slate-600 border border-slate-200"
                                  }`}>
                                    {details.geomType}
                                  </span>
                            ...
                            <div className={`text-[10.5px] leading-relaxed font-mono ${isDarkMode ? "text-slate-400" : "text-slate-605"}`}>
          ...
          <div className={`rounded-xl shadow-2xl w-full max-w-lg md:max-w-xl overflow-hidden flex flex-col max-h-[90vh] border ${
              ...
              <button
                ...
                className={`p-1.5 rounded-lg transition-colors text-[10px] font-bold px-3 cursor-pointer ${
                  isDarkMode 
                    ...
                    : "text-slate-700 hover:text-slate-950 bg-slate-200 hover:bg-slate-300"
                }`}
              >
                Tutup [×]
              </button>
            ...
            <div className="p-5 overflow-y-auto flex-1 flex flex-col gap-5 custom-scrollbar text-xs">
              ...
              <div className={`p-3 rounded-lg border flex items-center justify-between ${
                ...
                <div className="flex flex-col">
                  <span className={`text-[10px] uppercase tracking-wider font-bold ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>{t('mapLegend.layerName', 'Nama Layer')}</span>
                  <span className={`text-xs font-semibold ${isDarkMode ? "text-emerald-350" : "text-emerald-700"}`}><AutoTranslatedText text={layer.name} inline /></span>
                </div>
                <span 
                  className={`text-[9px] font-mono px-2 py-1 rounded border uppercase ${
                    isDarkMode 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className={`p-3 rounded-lg border flex flex-col gap-1.5 font-mono ${
                  isDarkMode ? "bg-slate-950/25 border-white/5" : "bg-slate-50/50 border-slate-200"
                }`}>
                  <span className={`text-[9px] uppercase font-bold text-left ${isDarkMode ? "text-slate-500" : "text-slate-500"}`}>{t('mapLegend.fileInfo', 'Informasi File')}</span>
                  <div className="flex items-center justify-between">
                    <span className={isDarkMode ? "text-slate-400" : "text-slate-500 font-medium"}>{t('mapLegend.uploadDate', 'Tgl Unggah:')}</span>
                  ...
                  <div className="flex items-center justify-between">
                    <span className={isDarkMode ? "text-slate-400" : "text-slate-500 font-medium"}>{t('mapLegend.totalFeatures', 'Total Fitur:')}</span>
                    <span className={`font-mono font-bold px-1.5 py-0.5 rounded ${
                      isDarkMode ? "text-slate-200 bg-white/5" : "text-slate-800 bg-slate-100 border border-slate-200"
                    }`}>
                      {featureCount}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className={isDarkMode ? "text-slate-400" : "text-slate-500 font-medium"}>{t('mapLegend.mapStatus', 'Status Peta:')}</span>
                    <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${
                      layer.isActive 
                        ? isDarkMode ? "bg-emerald-500/20 text-emerald-400" : "bg-emerald-50 text-emerald-700 border border-emerald-250 font-bold" 
                ...
                <div className={`p-3 rounded-lg border flex flex-col gap-1.5 font-mono ${
                  isDarkMode ? "bg-slate-950/20 border-white/5" : "bg-slate-50/50 border-slate-200"
                }`}>
                  <span className={`text-[9px] uppercase font-bold text-left ${isDarkMode ? "text-slate-500" : "text-slate-500"}`}>{t('mapLegend.visualStyle', 'Gaya Visual')}</span>
                  <div className="flex items-center justify-between">
                    <span className={isDarkMode ? "text-slate-400" : "text-slate-500 font-medium"}>{t('mapLegend.strokeColor', 'Warna Garis:')}</span>
                    <div className="flex items-center gap-1.5">
                      <span className="h-3 w-3 rounded-full border border-white/25" style={{ backgroundColor: layer.color }}></span>
                      <span className={`font-mono font-semibold ${isDarkMode ? "text-slate-300" : "text-slate-705"}`}>{layer.color}</span>
                    </div>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className={isDarkMode ? "text-slate-400" : "text-slate-500 font-medium"}>{t('mapLegend.strokeWidth', 'Lebar Garis:')}</span>
                    <span className={`font-mono font-bold ${isDarkMode ? "text-slate-200" : "text-slate-800"}`}>{layer.lineWidth || 2} px</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className={isDarkMode ? "text-slate-400" : "text-slate-500 font-medium"}>{t('mapLegend.opacity', 'Opasitas:')}</span>
              ...
              <div className={`p-3 rounded-lg border flex flex-col gap-1.5 shadow-inner ${
                ...
                <span className={`text-[9px] uppercase font-mono font-bold border-b pb-1 mb-1 ${
                  isDarkMode ? "text-slate-500 border-emerald-500/10" : "text-emerald-800 border-emerald-300/40"
                }`}>{t('mapLegend.spatialStats', 'Statistik & Analisis Spasial (Turf.js)')}</span>
                
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5">
                    <div className="flex items-center justify-between">
                      <span className={isDarkMode ? "text-slate-400" : "text-slate-606 font-medium"}>{t('mapLegend.totalArea', 'Total Luasan:')}</span>
                      <span className={`font-mono font-bold ${isDarkMode ? "text-emerald-400" : "text-emerald-700"}`}>{totalAreaHa.toFixed(2)} Ha</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className={isDarkMode ? "text-slate-400" : "text-slate-600 font-medium"}>{t('mapLegend.totalLength', 'Total Panjang/Keliling:')}</span>
                      <span className={`font-mono font-bold ${isDarkMode ? "text-emerald-400" : "text-emerald-700"}`}>{totalLengthKm.toFixed(2)} KM</span>
                    </div>
                  </div>
                  
                  <div className="flex flex-col gap-1.5">
                    <div className="flex items-center justify-between">
                      <span className={isDarkMode ? "text-slate-400" : "text-slate-600 font-medium"}>{t('mapLegend.featureDensity', 'Kepadatan (Feature/Ha):')}</span>
                      <span className={`font-mono font-bold ${isDarkMode ? "text-emerald-400" : "text-emerald-700"}`}>{densityFeaturesPerHa.toLocaleString("id-ID", {maximumFractionDigits: 4})}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className={isDarkMode ? "text-slate-400" : "text-slate-600 font-medium"}>{t('mapLegend.majorGeometry', 'Geometri Mayor:')}</span>
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 text-xs font-bold rounded-t-md transition-colors cursor-pointer ${
                      activeTab === "attributes" 
                        ? isDarkMode 
                          ? "bg-slate-800 text-emerald-400 border-emerald-500 border-b-2" 
                ...
                <div className={`p-3 rounded-lg border min-h-[140px] max-h-[220px] overflow-y-auto custom-scrollbar flex-1 ${
                  ...
                  {activeTab === "attributes" ? (
                    <div className="flex flex-col gap-2.5">
                      <p className={`text-[10px] ${isDarkMode ? "text-slate-400" : "text-slate-505 font-medium"}`}>
                      ...
                      {availableProperties.length === 0 ? (
                        <div className="italic text-center py-4 text-slate-500">{t('mapLegend.noAttributes', 'Tidak ada atribut data (properties kosong)')}</div>
                      ) : (
                        <div className="grid grid-cols-1 gap-1.5">
                          {Object.entries(firstFeatureProps).map(([key, val]) => (
                            <div key={key} className={`flex items-start justify-between py-1 border-b font-mono text-[10.5px] ${
                              ...
                              <span className={`truncate max-w-[250px] ${isDarkMode ? "text-slate-400" : "text-slate-600 font-medium"}`} title={typeof val === "object" ? JSON.stringify(val) : String(val)}>
                    ...
                    <div className="w-full flex flex-col gap-2 h-full min-h-[160px]">
                      <p className={`text-[10px] ${isDarkMode ? "text-slate-400" : "text-slate-500 font-medium"}`}>Distribusi tipe fitur geometri dalam layer spasial ini:</p>
                      {chartData.length === 0 ? (
                    ...
                    <div className="relative h-full flex flex-col">
                      <div className={`text-[10px] mb-1 flex justify-between items-center selection:bg-transparent ${isDarkMode ? "text-slate-500" : "text-slate-550 font-bold"}`}>
