    ...
    <div className="relative w-full h-full flex items-center justify-center">
      {!isLoaded && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/60 gap-3 z-20">
          ...
          <span className="text-slate-600 dark:text-slate-400 text-xs font-semibold uppercase tracking-wider">Memuat Foto...</span>
        </div>
      )}
      <img
        src={src}
      ...
      {images.length > 1 && (
        <>
          ...
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-black/50 px-6 py-3 rounded-full z-50">
            <button onClick={togglePlay} className="text-white hover:text-emerald-700 dark:hover:text-emerald-400 transition-colors">
