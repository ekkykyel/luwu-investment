...
export default function UploadGeoJsonPanel({ onUploadSuccess, isDarkMode }: UploadGeoJsonProps) {
  ...
  const downloadSampleGeoJson = () => {
      ...
      "features": [
        {
          "type": "Feature",
          "properties": {
            "Nama Kawasan": "Kawasan Industri Logistik Bua - Luwu",
            "Luas (Ha)": "320 Hektar",
    ...
    triggerDownload(sample, "bua_industrial_zone_sample.geojson");
  };
        ...
        <div className={`flex items-center justify-between border-b pb-4 ${isDarkMode ? "border-white/5" : "border-slate-200"}`}>
          <div className="flex items-center gap-3">
            <div className={`p-1.5 rounded-lg ${isDarkMode ? "bg-emerald-500/10" : "bg-emerald-50 border border-emerald-100"}`}>
              <Layers className={`h-4.5 w-4.5 ${isDarkMode ? "text-emerald-400" : "text-emerald-700"}`} />
            </div>
        ...
        <div className={`flex gap-1.5 p-1 rounded-xl border text-[10px] font-bold uppercase tracking-wider font-mono ${
          ...
          <button
            ...
            className={`flex-1 py-2 rounded-lg transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
              activeTab === "upload" 
                ? (isDarkMode ? "bg-slate-800/80 shadow-md text-emerald-400 border border-emerald-500/20" : "bg-emerald-600 shadow-md text-white border border-emerald-600") 
            ...
            className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all flex flex-col items-center justify-center ${
              dragOver 
                ...
                : file 
                  ...
                  : isDarkMode ? "border-slate-800 hover:border-slate-700 bg-slate-950/40 text-slate-300" : "border-slate-300 hover:border-slate-400 bg-slate-50/50 text-slate-700"
            }`}
          >
            <input
              type="file"
            ...
            <label htmlFor="geojson-input-elem" className="cursor-pointer flex flex-col items-center gap-2">
              <FileCode className={`h-8 w-8 ${file ? 'text-emerald-400' : (isDarkMode ? 'text-slate-500' : 'text-slate-400')}`} />
              {file ? (
                <div className="flex flex-col gap-0.5">
                  <span className={`text-xs font-semibold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{file.name}</span>
                  <span className="text-[9px] text-emerald-400 uppercase font-mono">{t('spatial.perfectGeometry')}</span>
                </div>
              ) : (
                <div className="flex flex-col gap-0.5">
                  <span className={`text-xs font-medium ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>{t('spatial.dragDropGeoJSON')}</span>
                  <span className={`text-[9px] font-mono ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Ekstensi .geojson atau .json standar (Maksimal 15MB)</span>
                </div>
              )}
            </label>
          </div>
          ...
          {!file && (
            <div className="flex flex-col items-center gap-2">
              ...
              <button 
                ...
                className="text-[10px] text-emerald-400 hover:text-emerald-300 font-medium underline cursor-pointer"
              >
              ...
              <div className="flex flex-col gap-1.5 font-sans w-full">
                ...
                {validationResult.isValid ? (
                  <div className="flex flex-col gap-1.5">
                    <div className="grid grid-cols-2 gap-2 font-mono text-[9px] text-slate-400 bg-slate-900/90 p-2 rounded-lg border border-slate-800">
                    ...
                    {validationResult.topologyIssues.length > 0 && (
                      <div className="text-[9.5px] text-amber-400 leading-normal bg-amber-950/10 border border-amber-500/20 p-2 rounded-lg font-mono">
                  ...
                  <p className="text-[10px] text-rose-400 leading-tight bg-slate-900 p-2 rounded-lg border border-slate-800 font-mono">
        ...
        <div className="flex flex-col gap-4 animate-fade-in">
          {!rawGeoJson ? (
            <div className="text-center py-6 text-slate-500 flex flex-col items-center gap-2">
              <Settings className="h-8 w-8 text-slate-700 animate-pulse" />
              <p className="text-xs">Unggah berkas GeoJSON terlebih dahulu untuk menjalankan Topology & Repair Suite, atau muat model demo!</p>
              <button
                type="button"
                onClick={loadComplexDemoPolygon}
                className="mt-2 text-[10px] bg-slate-950 hover:bg-slate-900 text-emerald-400 border border-slate-800 px-3 py-1 rounded-md font-bold cursor-pointer transition-all"
              >
            ...
            <div className="flex flex-col gap-4 text-xs">
              ...
              <div className="p-3.5 bg-slate-950 rounded-xl border border-slate-850 flex flex-col gap-2.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[8px] uppercase bg-emerald-950 text-emerald-400 border border-emerald-500/20 px-1 py-0.5 rounded font-mono">
                    Safe Mutator
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2 justify-between">
                  <span className="text-[9.5px] font-mono text-slate-400">Status: {isRepaired ? <span className="text-emerald-400 font-bold">Repelled & Saved</span> : <span className="text-amber-400">Menunggu Analisis</span>}</span>
                  <button
                    type="button"
                  ...
                  {versions.map((ver) => (
                    ...
                    >
                      <div className="flex flex-col gap-0.5">
                        ...
                        <span className="text-[8px] text-slate-500 font-mono">{ver.timestamp} WIB</span>
                      </div>
                      <span className="text-[8.5px] bg-slate-950 px-1.5 py-0.5 rounded font-mono text-slate-400">
                        {ver.vertexCount} pts
                      </span>
                    </div>
                  ))}
              ...
              <div className="p-3.5 bg-slate-950 rounded-xl border border-slate-850 flex flex-col gap-2 font-mono">
                <span className="text-[10px] text-emerald-400 font-bold uppercase tracking-wider block">Diagnostics / Repair Output:</span>
                <div className="bg-black/95 text-[9px] text-slate-400 p-2.5 rounded-lg border border-slate-900 overflow-y-auto font-mono leading-relaxed h-20 space-y-1">
