...
export const LUWU_DISTRICT_HYDROLOGY: Record<string, LuwuDistrictRiverData> = {
  ...
  "basse sangtempe utara": {
    kecamatanKey: "basse sangtempe utara",
...
const KECAMATAN_ALIAS_MAP: Record<string, string> = {
  ...
  // Bastem series
  "basse sangtempe": "basse sangtempe",
  "bassesangtempe": "basse sangtempe",
  "bastemutara": "basse sangtempe utara",
  "bastem utara": "basse sangtempe utara",
  "bassesangtempeutara": "basse sangtempe utara",
  "bassesangtempe utara": "basse sangtempe utara"
};
...
export function getHydrologyByDistrictName(
  ...
  // 1. Direct Exact Alias Lookup (Priority: Exact Match First)
  ...
  // 2. Direct Exact key match in LUWU_DISTRICT_HYDROLOGY
  if (normalizedQuery) {
    for (const key of Object.keys(LUWU_DISTRICT_HYDROLOGY)) {
      ...
      if (normKey === normalizedQuery || normName === normalizedQuery) {
        return data;
      }
    }
  }

  // 3. Sorted Longest-Alias Match (Ensures 'Bua Ponrang' or 'Ponrang Selatan' matches before 'Bua' or 'Ponrang')
  const sortedAliases = Object.entries(KECAMATAN_ALIAS_MAP).sort((a, b) => b[0].length - a[0].length);
  for (const [alias, targetKey] of sortedAliases) {
    const normAlias = normalizeDistrictName(alias);
    if (normAlias.length >= 4 && (normalizedQuery.includes(normAlias) || combinedText.includes(normAlias))) {
      if (LUWU_DISTRICT_HYDROLOGY[targetKey]) {
        return LUWU_DISTRICT_HYDROLOGY[targetKey];
      }
    }
